...
def list_sources():
    ...
    print("[Sources]", resp.status_code, resp.text)
    return resp.json()
...
def create_session(source_name):
    ...
    print("[Session]", resp.status_code, resp.text)
    return resp.json()

# Jules Source 등록/업데이트 함수
def register_source(owner, repo, env_info=None):
    ...
    print("[Register Source]", resp.status_code)
    try:
        result = resp.json()
        print(result)
        return result
    except Exception:
        print("[Register Source] 응답 본문:", resp.text)
        return None
...
def get_git_origin_and_branch():
    ...
    # https://github.com/owner/repo(.git) → owner, repo 추출
    try:
        ...
        def list_sources():
            print(f"[DEBUG] Fetching sources from {BASE_URL}/sources...")
            resp = requests.get(f"{BASE_URL}/sources", headers=headers)
            print("[DEBUG] [Sources Status]", resp.status_code)
            try:
                data = resp.json()
                with open("sources_debug.json", "w", encoding="utf-8") as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
                print("[DEBUG] sources_debug.json 파일로 전체 응답 저장 완료")
                sources = data.get("sources", [])
                print(f"[DEBUG] [Sources Count] {len(sources)}")
                for idx, s in enumerate(sources):
                    print(f"  [DEBUG] [{idx}] name={s.get('name')} githubRepo={s.get('githubRepo')} id={s.get('id')}")
                return data
            except Exception:
                print("[DEBUG] [Sources Error Body]", resp.text)
                return {}
