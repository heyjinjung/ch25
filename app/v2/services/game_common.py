"""V2 `game_common` implementation.

Provides shared game logic (logging, DDA, season pass hooks) without legacy dependencies.
"""
...
def log_game_play(ctx: GamePlayContext, db: Session, result_payload: dict[str, Any]) -> None:
    ...
    # Bridges (failures are non-blocking)
...
def _publish_internal_game_result(ctx: GamePlayContext, db: Session, result_payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        V2EventService().publish_internal_game_result(data)
    except Exception:
        pass
...
def should_apply_dda(user_id: int, settings) -> bool:
    if not (settings.ch25_dda_enabled and settings.ch25_intervention_enabled):
        return False
...
def _record_dda_outcome_if_needed(ctx: GamePlayContext, result_payload: dict[str, Any]) -> None:
    ...
    record_dda_outcome(ctx.user_id, is_win, applied, settings)
