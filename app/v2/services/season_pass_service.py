"""V2 Season pass domain service implementation.
...
class V2SeasonPassService:
        ...
        for level in new_levels:
            ...
            if level.auto_claim:
                ...
                try:
                    ...
                    self.logger.error(f"Reward delivery failed for user {user_id} level {level.level}", exc_info=True)
                rewards.append(
                    {
                        "level": level.level,
                        "reward_type": level.reward_type,
    ...
    def claim_reward(self, db: Session, user_id: int, level: int, now: date | datetime | None = None) -> dict:
        ...
        try:
            ...
            self.logger.error(f"Manual reward delivery failed for user {user_id} level {level}", exc_info=True)
        ...
        for level in new_levels:
            ...
            if level.auto_claim:
                ...
                try:
                    ...
                    self.logger.error(f"Bonus XP reward delivery failed for user {user_id} level {level.level}", exc_info=True)
                rewards.append(
                    {
                        "level": level.level,
                        "reward_type": level.reward_type,
