...
class V2RetentionInterventionService:
    """Resolve intervention rewards and log ROI evidence (V2)."""
    ...
    def resolve_intervention(self, db: Session, *, user_id: int, event_type: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        group = self._assign_experiment_group(user_id, settings)
        if group == "CONTROL":
            result = {
                "eligible": False,
    ...
    def _log_roi(self, db: Session, user_id: int, predicted_ltv: float, reward_amount: int, event_type: str, reward_type: str) -> float:
        ...
        roi_percent = 0.0
        if marketing_cost > 0:
            roi_percent = float((predicted_ltv / marketing_cost) * 100)

        log_row = V2RetentionRoiLog(
            user_id=user_id,
            predicted_ltv=predicted_ltv,
    ...
    def _touch_last_intervention(self, db: Session, user_id: int) -> None:
        row = db.query(V2UserRetentionState).filter(V2UserRetentionState.user_id == user_id).first()
        if row is None:
            row = V2UserRetentionState(user_id=user_id)
    ...
    def _assign_experiment_group(user_id: int, settings) -> str:
        ...
        digest = __import__("hashlib").sha1(f"{seed}:{user_id}".encode("utf-8")).hexdigest()
