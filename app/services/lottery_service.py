...
class LotteryService:
    ...
    def play(self, db: Session, user_id: int, now: date | datetime) -> LotteryPlayResponse:
        ...
        if chosen.reward_type in {"POINT", "CC_POINT"} and chosen.reward_amount and chosen.reward_amount >= 1000:
            try:
                from app.services.feed_service import FeedService
                FeedService().check_and_publish_jackpot(db, user_id, "LOTTERY", chosen.reward_amount)
        ...
        mission_service.update_progress(user_id, "PLAY_LOTTERY")  # 복권 게임 개별
        streak_info = mission_service.get_streak_info(user_id)

        total_earn = 0
        point_reward_amount = 0
        if chosen.reward_type in {"POINT", "CC_POINT"} and chosen.reward_amount:
        ...
        return LotteryPlayResponse(
            ...
            game_data={
                "collection_piece": chosen.reward_type.replace("PUZZLE_", "") if "PUZZLE_" in chosen.reward_type else None
            }
        )
