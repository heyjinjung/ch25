...
class MissionService:
    ...
    def ensure_login_progress(self, user_id: int):
        ...
        try:
            # This will iterate through all active LOGIN missions and update/create progress.
            self.update_progress(user_id, "LOGIN", delta=1)
        except Exception as e:
            # We don't want to block login if mission update fails, but we should log it.
            print(f"Update login progress failed: {e}")

        # 2. Handle NEW_USER specific logic (Day 2+ jump)
        # The standard update_progress only gives +1. 
        # For NEW_USER_LOGIN_DAY2, if target is 2 and user is on their 2nd day, we jump to completion.
        from app.models.mission import Mission, MissionCategory, UserMissionProgress
        new_user_login_missions = self.db.query(Mission).filter(
            Mission.action_type == "LOGIN",
            Mission.category == MissionCategory.NEW_USER,
            Mission.is_active == True
        ).all()

        for mission in new_user_login_missions:
            reset_date = self.get_reset_date_str(mission.category)
            progress = self.db.query(UserMissionProgress).filter(
                UserMissionProgress.user_id == user_id,
                UserMissionProgress.mission_id == mission.id,
            ...
            if progress and not progress.is_completed and progress.current_value < mission.target_value:
                ...
                if user.first_login_at:
                    ...
                    if now_kst > first_kst:
                        progress.current_value = mission.target_value # Reach completion (e.g. 2)
                        progress.is_completed = True
                        progress.completed_at = datetime.utcnow()
