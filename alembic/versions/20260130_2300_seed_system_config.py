...
def upgrade() -> None:
    ...
    if not _row_exists(conn, "feature_schedule", 1):
        conn.execute(text("""
            INSERT INTO feature_schedule (id, date, feature_type, is_active, created_at, updated_at)
            VALUES (1, '2026-01-23', 'ROULETTE', 1, NOW(), NOW())
        """))
    ...
    if not _row_exists_by_key(conn, "app_ui_config", "`key`", "v2_shop_products"):
        ...
        conn.execute(text("""
            INSERT INTO app_ui_config (`key`, value_json, created_at, updated_at)
            VALUES ('v2_shop_products', :json_val, NOW(), NOW())
    ...
    if not _row_exists_by_key(conn, "app_ui_config", "`key`", "shop_products"):
        ...
        conn.execute(text("""
            INSERT INTO app_ui_config (`key`, value_json, created_at, updated_at)
            VALUES ('shop_products', :json_val, NOW(), NOW())
