...
export const fetchAdminUiConfig = async (key: string): Promise<AdminUiConfigResponse> => {
  const response = await adminApi.get<AdminUiConfigResponse>(`/api/admin/ui-config/${encodeURIComponent(key)}`);
...
export const upsertAdminUiConfig = async (
  ...
  const response = await adminApi.put<AdminUiConfigResponse>(`/api/admin/ui-config/${encodeURIComponent(key)}`,
    payload
  );
  return response.data;
};
