...
const UserSegmentsPage: React.FC = () => {
  ...
  const [recommendedSegmentFilter, setRecommendedSegmentFilter] = useState<
    string | null
  >(null);
  ...
  const { data: segments, isLoading } = useQuery({
    ...
    queryFn: () =>
      fetchUserSegments(searchTerm ? { identifier: searchTerm } : undefined),
  });
  ...
  const filteredSegments = sortedSegments.filter((s) => {
    ...
    if (
      recommendedSegmentFilter &&
      s.recommended_segment !== recommendedSegmentFilter
    )
      return false;
  ...
  const getSortIcon = (key: SortKey) => {
    if (sortConfig?.key !== key)
      return <RefreshCw className="h-3 w-3 opacity-0 group-hover:opacity-30" />;
    return sortConfig.direction === "asc" ? (
      <ChevronUp className="h-3 w-3 text-admin-brand" />
    ) : (
      <ChevronDown className="h-3 w-3 text-admin-brand" />
    );
  };

  const updateMutation = useMutation({
    mutationFn: (payload: { user_id: number; segment: string }) =>
      upsertUserSegment(payload),
    onSuccess: () => {
  ...
  const getSegmentBadge = (segment: string) => {
    const colors: Record<string, string> = {
      ...
      WHALE:
        "ring-1 ring-inset ring-purple-500/30 text-purple-400 bg-purple-500/5",
      ACTIVE:
        "ring-1 ring-inset ring-emerald-500/30 text-emerald-400 bg-emerald-500/5",
      INACTIVE:
        "ring-1 ring-inset ring-zinc-700/30 text-zinc-500 bg-zinc-500/5",
    ...
    <section className="admin-page-container space-y-10 pb-20">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          <h1 className="text-admin-title text-admin-text-primary">
            세그먼트 관리
          </h1>
        </div>
        <button
        ...
        >
          ...
          <div className="flex items-end justify-between">
            <p className="text-3xl font-black text-amber-500">
              {segments?.filter((s) => s.segment === "VIP").length || 0}
            </p>
        ...
        >
          ...
          <div className="flex items-end justify-between">
            <p className="text-3xl font-black text-emerald-400">
              {segments?.filter((s) => s.segment === "ACTIVE").length || 0}
            </p>
        ...
        >
          ...
          <div className="flex items-end justify-between">
            <p className="text-3xl font-black text-rose-500">
              {segments?.filter((s) => s.segment === "CHURN").length || 0}
            </p>
      ...
      <div className="admin-card-premium p-6 space-y-6">
        <div className="flex flex-wrap items-end gap-6">
          ...
          <div className="w-48">
            ...
            <select
              ...
              onChange={(e) =>
                setRecommendedSegmentFilter(e.target.value || null)
              }
              className="admin-input h-10 w-full text-sm"
            >
              <option value="">전체 보기</option>
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-500">
          ...
          <span>
            필터링된 결과:{" "}
            <b className="text-admin-brand">{filteredSegments.length}</b> /{" "}
            {segments?.length || 0} 명
          </span>
        </div>
      </div>
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 z-10 bg-zinc-900 border-b border-zinc-800">
                <tr>
                  <th
                    className="px-4 py-3 text-sm font-bold text-zinc-400 uppercase tracking-wider cursor-pointer group hover:text-zinc-200"
                  ...
                  <th
                    className="px-4 py-3 text-sm font-bold text-zinc-400 uppercase tracking-wider cursor-pointer group hover:text-zinc-200"
                    onClick={() => handleSort("recommended_segment")}
                  >
                    <div className="flex items-center gap-1">
                  ...
                  <th
                    className="px-4 py-3.5 text-right text-sm font-bold text-zinc-400 cursor-pointer group hover:text-zinc-200"
                  ...
                  <th className="px-4 py-3.5 text-center text-sm font-bold text-zinc-400">
                    액션
                  </th>
                </tr>
              </thead>
              <tbody>
                {filteredSegments.length === 0 ? (
                  <tr>
                    <td
                      colSpan={7}
                      className="px-4 py-10 text-center text-admin-text-muted text-sm"
                    >
                      조회된 회원이 없습니다.
                    </td>
                  </tr>
                  ...
                  filteredSegments.map((seg: AdminUserSegmentRow) => (
                    <tr key={seg.user_id} className="admin-td group">
                      <td className="px-4 py-4 font-mono text-sm text-admin-text-primary">
                        {seg.user_id}
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex flex-col gap-1">
                          <span className="text-admin-text-primary font-bold text-base">
                            {seg.nickname || "-"}
                          </span>
                          <span className="text-zinc-400 text-sm">
                            @{seg.telegram_username || seg.external_id}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        {editingUserId === seg.user_id ? (
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-sm font-bold uppercase ${getSegmentBadge(seg.segment)}`}
                          >
                            {seg.segment}
                          </span>
                        )}
                      ...
                      <td className="px-4 py-4">
                        {seg.recommended_segment ? (
                          <div className="flex flex-col gap-0.5">
                            <span
                              className={`px-2 py-0.5 rounded-full text-sm font-bold uppercase w-fit ${getSegmentBadge(seg.recommended_segment)}`}
                            >
                              {seg.recommended_segment}
                            </span>
                            <span className="text-sm text-zinc-400">
                              {seg.recommended_rule_name}
                            </span>
                          </div>
                        ) : (
                      ...
                      <td className="px-4 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <span className="text-zinc-300 text-sm tabular-nums">
                            {seg.last_login_at
                              ? new Date(seg.last_login_at).toLocaleDateString()
                              : "-"}
                          </span>
                        </div>
                      </td>
