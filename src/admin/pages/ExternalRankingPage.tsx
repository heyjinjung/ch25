...
const formatKstCompact = (value?: string) => {
  ...
  return `${month}??{day}??{hh}??{mm}Î∂ÑÍπåÏßÄ`;
};
...
const ExternalRankingPage: React.FC = () => {
  ...
  const upsertMutation = useMutation({
    ...
    onSuccess: (res) => {
      // Ï¶âÏãú UI??Î∞òÏòÅ ???ÔøΩÎ≤Ñ ?ÔøΩÏù¥?ÔøΩÎèÑ ?ÔøΩÎ°úÍ≥†Ïπ®
      if (res?.items) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_res, userId) => {
      // ??ÔøΩÔøΩ ÏßÅÌõÑ Î™©Î°ù?ÔøΩÏÑú ?ÔøΩÍ±∞?ÔøΩÍ≥† ?ÔøΩÎ≤Ñ ?ÔøΩÏù¥?ÔøΩÎèÑ ?ÔøΩÎ°úÍ≥†Ïπ®
  ...
  const resolveOne = async (index: number) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail || err?.message || "Îß§Ïπ≠ ?§Ìå®";
      ...
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-8">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold text-admin-text-base tracking-tight uppercase">
            ?∏Î? ??Çπ Í¥ÄÎ¶?<span className="text-admin-brand/40">External Ranking</span>
          </h1>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {upsertMutation.isPending ? "?Ä??Ï§?.." : "?ÑÏ≤¥ ?Ä??(Save All)"}
          </button>
        </div>
      </header>

      {isLoading && (
        <div className="admin-card p-4 text-admin-body text-admin-text-secondary">Î∂àÎü¨?§Îäî Ï§?..</div>
      )}
      {isError && (
        <div className="admin-card p-4 text-admin-body text-red-200">Î∂àÎü¨?§Í∏∞ ?§Ìå®: {(error as Error).message}</div>
      )}

      {/* Transparent Search & Controls */}
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between px-2">
        <div className="flex-1 max-w-xl">
          <label className="text-xs font-black text-zinc-500 uppercase tracking-widest mb-3 block ml-1">Îß§Ïπ≠ ?∞Ïù¥??Í≤Ä??(Search)</label>
          ...
          <table className="admin-table sticky-header">
            <thead>
              <tr className="bg-zinc-900 border-b border-zinc-800">
                ...
                <th className="px-4 py-3.5 text-left text-sm font-bold text-zinc-400 uppercase tracking-widest">?îÎ†àÍ∑∏Îû® ?ïÎ≥¥ (TG Info)</th>
                <th className="px-4 py-3.5 text-left text-sm font-bold text-zinc-400 uppercase tracking-widest">?ÑÎ°ú??(Profile)</th>
                <th className="px-4 py-3.5 text-left text-sm font-black text-zinc-400 uppercase tracking-widest text-admin-brand">Îß§Ïπ≠?†Ï? (Matched)</th>
                <th className="px-4 py-3.5 text-left text-sm font-bold text-zinc-400 uppercase tracking-widest">ÏµúÏ¢Ö ?ôÍ∏∞??(Sync)</th>
                <th className="px-4 py-3.5 text-right text-sm font-bold text-zinc-400 uppercase tracking-widest cursor-pointer hover:text-zinc-200" onClick={() => toggleSort("deposit_amount")}>?ÖÍ∏à??(Deposit)</th>
                <th className="px-4 py-3.5 text-right text-sm font-bold text-zinc-400 uppercase tracking-widest cursor-pointer hover:text-zinc-200" onClick={() => toggleSort("play_count")}>?åÎ†à??(Plays)</th>
                <th className="px-4 py-3.5 text-center text-sm font-bold text-zinc-400 uppercase tracking-widest">Í¥ÄÎ¶?(Action)</th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-right relative group/cell">
                      ...
                      {/* If editing IS required, we should use a modal or a toggle. For now, assuming direct Input -> Text change based on "Read-Only Data Display" requirement. 
                            ...
                            "Deposit/Plays Í∞íÏùÑ ?ÖÎ†•Ï∞ΩÏóê???úÏàò ?çÏä§??Text)Î°?Î≥ÄÍ≤? implies strict read-only look.
              ...
              {rows.length === 0 && (
                <tr>
                  <td className="px-4 py-10 text-center text-admin-text-muted text-sm italic" colSpan={9}>
                    ?∞Ïù¥?∞Í? Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§. ?âÏùÑ Ï∂îÍ??òÏó¨ ?ÖÎ†•???úÏûë?òÏÑ∏??
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mt-12 px-2 bg-admin-sidebar/20 p-6 rounded-2xl border border-white/5">
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end mr-2">
            ...
            <span className="text-xs text-zinc-500 font-medium">Î≥ÄÍ≤ΩÎêú Î™®Îì† ?∞Ïù¥?∞Î? ?úÎ≤Ñ???ôÍ∏∞?îÌï©?àÎã§.</span>
          </div>
          <button
            onClick={saveAll}
            disabled={upsertMutation.isPending || !isDirty}
          ...
          >
            ...
            {upsertMutation.isPending ? "?ôÍ∏∞??Ï§?.." : "?ÑÏ≤¥ ?Ä??(Save All)"}
          </button>
        </div>
      </div>
    </section>
