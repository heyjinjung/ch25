...
const UserAdminPage: React.FC = () => {
  ...
  const itemCountText = useMemo(() => {
    if (sortedMembers.length === 0) return "0Í∞???™© ?úÏãú";
    const from = startIndex + 1;
    const to = Math.min(startIndex + itemsPerPage, sortedMembers.length);
    return `${from}-${to}/${sortedMembers.length}Í∞???™© ?úÏãú`;
  ...
  const resetPassword = (row: MemberRow) => {
    ...
    if (nextPassword.length < 4) {
      addToast("ÎπÑÎ?Î≤àÌò∏??ÏµúÏÜå 4???¥ÏÉÅ ?ÖÎ†•?òÏÑ∏??", "error");
      return;
    }
  ...
  const removeRow = (row: MemberRow) => {
    const ok = window.confirm("?ïÎßê ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?");
    if (!ok) return;
  ...
  const purgeRow = (row: MemberRow) => {
    const ok = window.confirm(
      "[?òÎìú ?ºÏ?] ?†Ï? Î∞?Í¥Ä??Í∏∞Î°ù(?îÎ†àÍ∑∏Îû®/Î°úÍ∑∏/?¥Î≤§??????Í∞Ä?•Ìïú Î≤îÏúÑ?êÏÑú ?ÑÎ? ??†ú?©Îãà?? ?òÎèåÎ¶????ÜÏäµ?àÎã§. ÏßÑÌñâ?†Íπå??"
    );
    if (!ok) return;
  ...
  const submitNewMember = (e: React.FormEvent) => {
    ...
    if (!nickname) {
      addToast("?âÎÑ§?ÑÏ? ?ÑÏàò?ÖÎãà??", "error");
      return;
    }
    ...
    // ?§ÌÅ¨Î¶∞ÏÉ∑ UX??ÎßûÏ∂∞: ?ÖÎ†• ?âÎÑ§?ÑÏùÑ external_idÎ°úÎèÑ ?¨Ïö©
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 text-zinc-100">
      ...
      <div className="flex-none p-4 border-b border-zinc-800 bg-zinc-900/50 backdrop-blur-sm space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold tracking-tight text-white">?åÏõê Í¥ÄÎ¶?(User Admin)</h2>
        ...
        <div className="w-full max-w-xl">
          <div className="relative group">
            ...
            <input
              type="text"
              placeholder="ID, ?âÎÑ§?? TG Username, ?§Î™Ö Í≤Ä??.."
              value={searchInput}
...

      {
        showAddForm && (
          <div className="admin-card p-6">
            <h3 className="text-admin-subtitle text-admin-text-primary">???åÏõê Ï∂îÍ?</h3>
            ...
            <form onSubmit={submitNewMember} className="mt-4 space-y-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-admin-border">
                  <div className="space-y-1">
                    <label htmlFor="real_name" className="admin-label">?§Î™Ö</label>
                    <input
                      id="real_name"
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label htmlFor="phone_number" className="admin-label">?∞ÎùΩÏ≤?/label>
                    <input
                      id="phone_number"
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label htmlFor="telegram_id" className="admin-label">?îÎ†àÍ∑∏Îû® ID (?´Ïûê)</label>
                    <input
                      id="telegram_id"
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label htmlFor="telegram_username" className="admin-label">?îÎ†àÍ∑∏Îû® ?âÎÑ§??(@?úÏô∏)</label>
                    <input
                  ...
                  <div className="space-y-1">
                    <label htmlFor="tags" className="text-xs font-medium text-gray-400">?úÍ∑∏ (?ºÌëú Íµ¨Î∂Ñ)</label>
                    <input
                      id="tags"
                      type="text"
                      ...
                      placeholder="VIP, Î≥¥ÎÑà?? ?†Í∑ú"
                    />
                  </div>
                  <div className="md:col-span-2 space-y-1">
                    <label htmlFor="memo" className="text-xs font-medium text-gray-400">Î©îÎ™®</label>
                    <textarea
                      id="memo"
                      value={newMember.memo}
                      onChange={(e) => setNewMember((p) => ({ ...p, memo: e.target.value }))}
                      className="w-full rounded-md border border-[#333333] bg-[#1A1A1A] p-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-[#91F402]"
                      placeholder="?πÏù¥?¨Ìï≠ ?ÖÎ†•"
                      rows={2}
                    />
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {createMutation.isPending ? "?Ä??Ï§?.." : "?Ä??}
                </button>
              </div>
            </form>
          </div>
      ...
      {
        isLoading && (
          <div className="rounded-lg border border-[#333333] bg-[#111111] p-4 text-gray-200">Î∂àÎü¨?§Îäî Ï§?..</div>
        )
      }
      {
        isError && (
        ...
        !isLoading && !isError && (
          <div className="flex-1 overflow-hidden relative flex flex-col">
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 z-10 bg-zinc-900 border-b border-zinc-800">
                  <tr>
                    ...
                    >
                      <div className="flex items-center gap-1">?âÎÑ§??renderSortIcon("nickname")}</div>
                    </th>
                    <th
                    ...
                    >
                      <div className="flex items-center gap-1">?àÎ≤®{renderSortIcon("level")}</div>
                    </th>
                    <th
                    ...
                    <th className="hidden md:table-cell px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-zinc-400">?§Î™Ö/?∞ÎùΩÏ≤?/th>
                    ...
                    <th className="hidden lg:table-cell px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-zinc-400">Î©îÎ™®/?úÍ∑∏</th>
                    <th className="hidden lg:table-cell px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-zinc-400">ÎπÑÎ?Î≤àÌò∏(V2 Î¶¨Î†â)</th>
                    <th className="px-4 py-3 text-center text-sm font-bold uppercase tracking-wider text-zinc-400">?°ÏÖò</th>
                  </tr>
                </thead>
                      ...
                      <td className="px-3 py-3 whitespace-nowrap text-center sm:px-4">
                        <div className="flex justify-center gap-1 sm:gap-3">
                          ...
                          <button
                            ...
                            title="Í∏àÍ≥† Í¥ÄÎ¶?(?¥Ïó≠/?îÏï°)"
                            aria-label="Í∏àÍ≥† Í¥ÄÎ¶?(?¥Ïó≠/?îÏï°)"
                          >
                          ...
                          <button
                            ...
                            title="?òÎìú ?ºÏ?(?ÑÏ†Ñ Ï¥àÍ∏∞??"
                            aria-label="?òÎìú ?ºÏ?(?ÑÏ†Ñ Ï¥àÍ∏∞??"
                          >
                            <Skull size={16} />
                          </button>
                          <button
            ...
            {sortedMembers.length === 0 && (
              <div className="py-8 text-center text-gray-400">Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§.</div>
            )}
            ...
            {totalPages > 0 && (
              <div className="flex flex-col gap-2 border-t border-[#333333] bg-[#1A1A1A] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <nav className="relative z-0 inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                  ...
                  >
                    <span className="sr-only">?¥Ï†Ñ</span>
                  ...
                  >
                    <span className="sr-only">?§Ïùå</span>
      ...
      <div className="w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-2xl border border-[#333333] bg-[#111111] shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
          {isLoading ? (
            ...
            <div className="py-20 text-center text-gray-500 font-medium">?úÏÑ±?îÎêú ÎØ∏ÏÖò???ÜÏäµ?àÎã§.</div>
          ) : (
              ...
              {missions.map((m) => (
                <div key={m.mission_id} className="rounded-xl border border-[#333333] bg-[#1A1A1A] p-4 flex flex-col gap-3 group hover:border-[#91F402]/30 transition-all">
                  ...
                  <div className="mt-2 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-400">ÏßÑÌñâ?? {m.current_value} / {m.target_value}</span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        <Plus size={14} />
                        {m.is_claimed ? "ÏßÄÍ∏??ÑÎ£å" : "?òÎèô ÏßÄÍ∏?}
                      </button>
                    </div>
                  </div>
                </div>
