...
const prizeSchema = z.object({
  ...
  label: z.string().min(1, "?ÅÌíàÎ™ÖÏùÑ ?ÖÎ†•?òÏÑ∏??),
  weight: z.number().int().nonnegative("Í∞ÄÏ§ëÏπò??0 ?¥ÏÉÅ?¥Ïñ¥???©Îãà??),
  stock: z.preprocess(normalizeStock, z.number().int().nonnegative("?¨Í≥†??0 ?¥ÏÉÅ?¥Ïñ¥???©Îãà??).nullable()),
  reward_type: z.string().min(1, "Î≥¥ÏÉÅ ?Ä?ÖÏùÑ ?†ÌÉù?òÏÑ∏??),
  reward_value: z.number().int().nonnegative("Î≥¥ÏÉÅ Í∞íÏ? 0 ?¥ÏÉÅ?¥Ïñ¥???©Îãà??),
  is_active: z.boolean().default(true),
});

const lotterySchema = z
  .object({
    name: z.string().min(1, "?¥Î¶Ñ???ÖÎ†•?òÏÑ∏??),
    is_active: z.boolean().default(false),
    max_daily_plays: z.number().int().nonnegative("0?¥Î©¥ Î¨¥Ï†ú?úÏûÖ?àÎã§"),
    prizes: z.array(prizeSchema).min(1, "?ÅÌíà?Ä 1Í∞??¥ÏÉÅ Ï∂îÍ??òÏÑ∏??),
  })
  .refine((value) => {
    const labels = value.prizes.map((p) => p.label.trim());
    return new Set(labels).size === labels.length;
  }, {
    message: "?ÅÌíàÎ™ÖÏù¥ Ï§ëÎ≥µ?????ÜÏäµ?àÎã§",
    path: ["prizes"],
  })
  .refine((value) => value.prizes.some((p) => p.is_active && p.weight > 0), {
    message: "?úÏÑ± ?ÅÌíà Ï§?Í∞ÄÏ§ëÏπòÍ∞Ä 0Î≥¥Îã§ ????™©??1Í∞??¥ÏÉÅ ?ÑÏöî?©Îãà??,
    path: ["prizes"],
  })
  .refine((value) => value.prizes.reduce((sum, p) => sum + p.weight, 0) > 0, {
    message: "?ÑÏ≤¥ Í∞ÄÏ§ëÏπò ?©Ï? 0Î≥¥Îã§ Ïª§Ïïº ?©Îãà??,
    path: ["prizes"],
  });
...
const mapErrorDetail = (error: unknown): string => {
  ...
  if (typeof detail === "string") {
    const map: Record<string, string> = {
      LOTTERY_CONFIG_NOT_FOUND: "Î≥µÍ∂å ?§Ï†ï??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.",
      INVALID_LOTTERY_WEIGHT: "Í∞ÄÏ§ëÏπò??0 ?¥ÏÉÅ?¥Ïñ¥???©Îãà??",
      INVALID_LOTTERY_STOCK: "?¨Í≥†??0 ?¥ÏÉÅ ?êÎäî ÎπàÏπ∏(Î¨¥Ï†ú???ÖÎãà??",
      DUPLICATE_PRIZE_LABEL: "?ÅÌíàÎ™ÖÏù¥ Ï§ëÎ≥µ?òÏóà?µÎãà??",
      NO_ACTIVE_PRIZE: "?úÏÑ± ?ÅÌíà(Í∞ÄÏ§ëÏπò>0)??1Í∞??¥ÏÉÅ ?ÑÏöî?©Îãà??",
      ZERO_TOTAL_WEIGHT: "?ÑÏ≤¥ Í∞ÄÏ§ëÏπò ?©Ï? 0Î≥¥Îã§ Ïª§Ïïº ?©Îãà??",
      INVALID_LOTTERY_CONFIG: "Î≥µÍ∂å ?§Ï†ï Í∞íÏù¥ ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§.",
    };
    return map[detail] ?? detail;
  }
  return (error as any)?.message ?? "?îÏ≤≠ Ï≤òÎ¶¨ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.";
};
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <span className="text-sm font-bold text-zinc-500 uppercase tracking-widest">Î≥µÍ∂å ?úÏä§???∞Ïù¥???ôÍ∏∞??Ï§?..</span>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-zinc-500 text-sm font-medium">
            ...
            <span className="text-zinc-300">Î≥µÍ∂å ?§Ï†ï</span>
          </div>
          <h1 className="text-2xl font-black text-white flex items-center gap-3 tracking-tight">
            <Trophy className="text-admin-brand" size={28} />
            Î≥µÍ∂å ?¥ÏòÅ Ïª§Îß®???ºÌÑ∞
          </h1>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-2">
          ...
          <h2 className="text-base font-black text-white uppercase tracking-wider">Î≥µÍ∂å ?¥ÏòÅ ?§Ï†ï Î™©Î°ù (Lottery Fleet)</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {(configs ?? []).length === 0 && (
            <div className="md:col-span-2 py-24 flex flex-col items-center justify-center border-2 border-dashed border-zinc-800 rounded-[32px] opacity-40 grayscale hover:grayscale-0 transition-all">
              ...
              <p className="text-sm font-bold text-zinc-500">?¥ÏòÅ Ï§ëÏù∏ Î≥µÍ∂å ?§Ï†ï???ÜÏäµ?àÎã§.</p>
              <p className="text-[10px] text-zinc-600 mt-1 uppercase tracking-widest">???§Ï†ï??Ï∂îÍ??òÏó¨ ?îÏßÑ??Í∞Ä?ôÌïò?∏Ïöî.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="admin-card w-full max-w-6xl max-h-[90dvh] flex flex-col shadow-admin-glow border-admin-brand/30 overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-white tracking-tight">
                    {editing ? "Î≥µÍ∂å ?îÏßÑ Î©îÏª§?àÏ¶ò ?òÏ†ï" : "?†Í∑ú Î≥µÍ∂å Î©îÏª§?àÏ¶ò Íµ¨Ï∂ï"}
                  </h3>
                  <p className="text-xs text-zinc-500 font-medium">{editing ? editing.name : "?∞Ïù¥?∞Î≤†?¥Ïä§???ôÍ∏∞?îÎê† ?àÎ°ú??Íµ¨ÏÑ±???ÖÎ†•?òÏÑ∏??}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={onSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">?îÏßÑ Í≥†Ïú† ?ùÎ≥Ñ Î™ÖÏπ≠</label>
                  <input
                    type="text"
                    ...
                    placeholder="?? 2024 ?†ÎÖÑ ?πÎ≥Ñ Î≥µÍ∂å"
                    {...form.register("name")}
                  />
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">?ºÏùº Ï¥?Ï∞∏Ïó¨ ?úÌïú (0=Î¨¥Ï†ú??</label>
                  <input
                    type="number"
              ...
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-zinc-900/40 border border-zinc-800">
                ...
                <span className="text-xs font-bold text-zinc-300">?îÏßÑ Ï¶âÏãú ?úÏÑ±??(Operation Status: Live)</span>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-black text-white uppercase tracking-widest">Î∞∞Ï∂ú ?ÅÌíà ?Ä Íµ¨ÏÑ± (Prize Pool)</h4>
                  </div>
                  <button
                    type="button"
                    onClick={() =>
                      ...
                      <div key={field.id} className="admin-card bg-zinc-900/20 border-zinc-800/80 p-5 space-y-4 hover:border-zinc-700 transition-all group">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
                          <div className="lg:col-span-3 space-y-1.5">
                            <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">?ÅÌíàÎ™?/label>
                            <input
                              type="text"
                          ...
                          <div className="lg:col-span-2 space-y-1.5">
                            <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">Í∞ÄÏ§ëÏπò</label>
                            <input
                              type="number"
                          ...
                          <div className="lg:col-span-2 space-y-1.5">
                            <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">?¨Í≥† (Empty=??</label>
                            <input
                              type="number"
                              className="w-full h-11 bg-zinc-900 border border-zinc-800 rounded-xl px-4 text-sm text-white font-mono focus:border-admin-brand outline-none"
                          ...
                          <div className="lg:col-span-2 flex items-center gap-2">
                            <div className="flex-1 space-y-1.5">
                              <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">Î≥¥ÏÉÅ ?§Ï†ï</label>
                                  ...
                                  >
                                    {!REWARD_TYPES.some((rt) => rt.value === form.watch(`prizes.${idx}.reward_type`)) && (
                                      <option value={form.getValues(`prizes.${idx}.reward_type`)}>
                                        ?†Ô∏è ?????ÜÏùå ({form.getValues(`prizes.${idx}.reward_type`)})
                                      </option>
                                    )}
                                    {REWARD_TYPES.map((rt) => (
                              ...
                              {!REWARD_TYPES.some((rt) => rt.value === form.watch(`prizes.${idx}.reward_type`)) && (
                                <p className="text-[9px] text-rose-400 font-bold ml-1">?ÑÏó≠ ?ôÍ∏∞?îÎêòÏßÄ ?äÏ? ?Ä?ÖÏûÖ?àÎã§. ?òÏ†ï???ÑÏöî?©Îãà??</p>
                              )}
                            </div>
                          </div>
            ...
            <div className="p-8 border-t border-zinc-800 bg-zinc-900/50 flex justify-end gap-4">
              ...
              >
                ...
                {editing ? "?úÏä§??Íµ¨ÏÑ± ?ÖÎç∞?¥Ìä∏" : "?úÏä§??Íµ¨ÏÑ± Ï¶âÏãú Î∞∞Ìè¨"}
              </button>
            </div>
          </div>
        </div>
