...
const loginSchema = z.object({
  username: z.string().min(1, "?ÑÏù¥?îÎ? ?ÖÎ†•?òÏÑ∏??"),
  password: z.string().min(1, "ÎπÑÎ?Î≤àÌò∏Î•??ÖÎ†•?òÏÑ∏??"),
});
...
const AdminLoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      setServerError(
        err.response?.data?.detail || "?∏Ï¶ù???§Ìå®?àÏäµ?àÎã§. ?êÍ≤© Ï¶ùÎ™Ö???ïÏù∏?òÏÑ∏??"
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-admin-bg flex items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-[420px] animate-in fade-in zoom-in-95 duration-700">
        <header className="text-center mb-10 space-y-3">
          ...
          <h1 className="text-3xl font-black text-admin-text-primary tracking-tight">Í¥ÄÎ¶¨Ïûê Î°úÍ∑∏??/h1>
        </header>
