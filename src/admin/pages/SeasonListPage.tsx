...
const seasonSchema = z
  .object({
    name: z.string().min(1, "?¥Î¶Ñ?Ä ?ÑÏàò?ÖÎãà??),
    start_date: z.string().min(1, "?úÏûë?ºÏùÑ ?ÖÎ†•?òÏÑ∏??),
    end_date: z.string().min(1, "Ï¢ÖÎ£å?ºÏùÑ ?ÖÎ†•?òÏÑ∏??),
    max_level: z.number().int().positive("ÏµúÎ? ?àÎ≤®?Ä 1 ?¥ÏÉÅ"),
    base_xp_per_stamp: z.number().int().positive("?§ÌÉ¨?ÑÎãπ XP??1 ?¥ÏÉÅ"),
    is_active: z.boolean().default(true),
  })
  .refine((value) => new Date(value.start_date) <= new Date(value.end_date), {
    message: "Ï¢ÖÎ£å?ºÏ? ?úÏûë???¥ÌõÑ?¨Ïïº ?©Îãà??,
    path: ["end_date"],
  });
    ...
    <section className="bg-[#09090b] min-h-screen pb-20">
      ...
      <div className="px-8 max-w-[1600px] mx-auto">
        <header className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white">?úÏ¶å Í¥ÄÎ¶?(Season Ops)</h1>
          </div>
          <button
            type="button"
            onClick={() => {
        ...
        {isLoading && (
          <div className="h-64 flex items-center justify-center gap-3 text-zinc-500 bg-[#18181b] rounded-2xl border border-white/5">
            ...
            <span className="text-sm font-medium">?∞Ïù¥??Î°úÎìú Ï§?..</span>
          </div>
        )}
        ...
        {isError && (
          <div className="h-64 flex items-center justify-center gap-3 text-rose-400 bg-[#18181b] rounded-2xl border border-rose-500/20">
            <AlertCircle size={20} />
            <span className="text-sm font-bold">?∞Ïù¥??Î°úÎìú ?§Ìå®: {(error as Error).message}</span>
          </div>
        )}
        ...
        {!isLoading && data && data.items.length === 0 && (
          <div className="h-64 flex flex-col items-center justify-center gap-4 text-zinc-500 bg-[#18181b] rounded-2xl border border-white/5 border-dashed">
            ...
            <p className="text-sm font-medium">?±Î°ù???úÏ¶å???ÜÏäµ?àÎã§.</p>
          </div>
        )}
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-zinc-900/50 border-b border-white/5">
                    <th className="pl-6 pr-4 py-3 text-sm font-bold text-zinc-500 uppercase tracking-wider">?úÏ¶å Î™?(ID)</th>
                    <th className="px-4 py-3 text-sm font-bold text-zinc-500 uppercase tracking-wider">?¥ÏòÅ Í∏∞Í∞Ñ (Time Gauge)</th>
                    <th className="px-4 py-3 text-sm font-bold text-zinc-500 uppercase tracking-wider">ÏµúÎ? LV</th>
                    <th className="px-4 py-3 text-sm font-bold text-zinc-500 uppercase tracking-wider">XP Rate</th>
                    <th className="px-4 py-3 text-sm font-bold text-zinc-500 uppercase tracking-wider">?ÅÌÉú</th>
                    <th className="px-4 py-3 text-sm font-bold text-zinc-500 uppercase tracking-wider text-center">?°ÏÖò</th>
                  </tr>
                </thead>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                addToast("?àÎ≤® ?ïÎ≥¥Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??", "error");
      ...
      {/* Main Season Modal */}
      {isModalOpen && (
        <ModalShell title={editingSeason ? "?úÏ¶å Í∏∞Î≥∏ ?§Ï†ï ?òÏ†ï" : "???úÏ¶å ?ùÏÑ±"} onClose={resetAndClose}>
          <form className="space-y-6 p-2" onSubmit={onSubmit}>
            ...
            <div className="bg-[#1e1e24] p-6 rounded-xl border border-white/5 space-y-6">
              <div>
                <label className={labelClass}>?úÏ¶å Î™ÖÏπ≠ <span className="text-rose-500">*</span></label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className={labelClass}>?úÏûë ?ºÏûê</label>
                  <input type="date" className={inputClass} {...form.register("start_date")} />
                </div>
                <div>
                  <label className={labelClass}>Ï¢ÖÎ£å ?ºÏûê</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className={labelClass}>ÏµúÎ? ?àÎ≤® (Max Level)</label>
                  <input type="number" className={inputClass} {...form.register("max_level", { valueAsNumber: true })} />
                </div>
                <div>
                  <label className={labelClass}>?§ÌÉ¨?ÑÎãπ ?çÎìù XP</label>
              ...
              <div className="pt-2">
                <label className="flex items-center gap-3 cursor-pointer group p-3 rounded-lg border border-white/5 bg-zinc-900/50 hover:bg-zinc-900 transition-colors">
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-zinc-200">?úÏ¶å ?úÏÑ±??(Active)</span>
                    <span className="text-xs text-zinc-500 mt-0.5">?úÏÑ±????Ï¶âÏãú ?¨Ïö©?êÏóêÍ≤??∏Ï∂ú?©Îãà??</span>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
              ...
              >
                ...
                {editingSeason ? "Î≥ÄÍ≤ΩÏÇ¨???Ä?? : "?úÏ¶å ?ùÏÑ±"}
              </button>
            </div>
          </form>
        </ModalShell>
      ...
      {isLevelModalOpen && levelEditingSeason && (
        <ModalShell
          title={`${levelEditingSeason.name} ???àÎ≤® Î¶¨Ïõå???§Ï†ï`}
          onClose={() => {
            setIsLevelModalOpen(false);
        ...
        >
          {isLevelLoading ? (
            <div className="h-40 flex items-center justify-center gap-3 text-zinc-500">
              <RefreshCw className="animate-spin h-5 w-5" />
              <span className="text-sm">?àÎ≤® Íµ¨ÏÑ±??Î∂àÎü¨?§Îäî Ï§?..</span>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between bg-indigo-500/10 p-5 rounded-xl border border-indigo-500/20">
                <div className="flex flex-col">
                  <span className="text-xs font-black text-indigo-400 uppercase tracking-widest">?§Ï†ï ?Ä??(Configuration Target)</span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-2xl font-black text-white">{levels.length}</span>
                    <span className="text-sm text-indigo-200/60 font-medium">Í∞??àÎ≤® (Levels)</span>
                  </div>
                </div>
                <button
                  type="button"
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-4 mt-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-zinc-500 uppercase tracking-wider">?àÎ≤® Î≥¥ÏÉÅ (Level Reward)</span>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div className="col-span-2">
                          <label className="text-[11px] font-black text-zinc-500 uppercase tracking-widest block mb-1.5 ml-0.5">Ï¢ÖÎ•ò (Type)</label>
                          <select
                        ...
                        <div>
                          <label className="text-[11px] font-black text-zinc-500 uppercase tracking-widest block mb-1.5 ml-0.5">?òÎüâ (Amt)</label>
                          <input
                            type="number"
