...
const gifticonBrands = [
  { value: "CC_COIN", label: "?®Ïî®ÏΩîÏù∏" },
  { value: "BAEMIN", label: "Î∞∞Î?" },
  { value: "COMPOSE", label: "Ïª¥Ìè¨Ï¶àÏª§?? },
  { value: "STARBUCKS", label: "?§Ì?Î≤ÖÏä§" },
  { value: "CU", label: "CU" },
  { value: "GS25", label: "GS25" },
  { value: "CUSTOM", label: "ÏßÅÏ†ë ?ÖÎ†•" },
] as const;
...
const segmentSchema = z.object({
  label: z.string().min(1, "?ºÎ≤®???ÖÎ†•?òÏÑ∏??),
  weight: z.number().int().nonnegative("Í∞ÄÏ§ëÏπò??0 ?¥ÏÉÅ"),
  reward_type: z.string().min(1, "Î≥¥ÏÉÅ ?Ä?ÖÏùÑ ?†ÌÉù?òÏÑ∏??),
  reward_value: z.number().int().nonnegative("Î≥¥ÏÉÅ Í∞íÏ? 0 ?¥ÏÉÅ"),
});
...
const rouletteSchema = z
  .object({
    ...
    max_daily_spins: z.number().int().nonnegative("0?¥Î©¥ Î¨¥Ï†ú??),
    segments: z.array(segmentSchema).length(6, "?∏Í∑∏Î®ºÌä∏??6Í∞úÍ? ?ÑÏöî?©Îãà??),
  })
  .refine((value) => value.segments.reduce((sum, seg) => sum + seg.weight, 0) > 0, {
    message: "Í∞ÄÏ§ëÏπò ?©Ï? 0Î≥¥Îã§ Ïª§Ïïº ?©Îãà??,
    path: ["segments"],
  });
...
const mapErrorDetail = (error: unknown): string => {
  ...
  if (typeof detail === "string") {
    const map: Record<string, string> = {
      INVALID_ROULETTE_CONFIG: "Î£∞Î†õ ?§Ï†ï Í∞íÏù¥ ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§.",
      ROULETTE_CONFIG_NOT_FOUND: "Î£∞Î†õ ?§Ï†ï??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.",
    };
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-zinc-500 text-xs font-medium">
            ...
            <span className="text-zinc-300">Î£∞Î†õ ?§Ï†ï</span>
          </div>
      ...
      {isLoading ? (
        <div className="admin-card p-12 flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-sm font-medium text-zinc-500">?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§ëÏûÖ?àÎã§...</p>
        </div>
      ) : isError ? (
        <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-6 flex items-center gap-4">
          <AlertCircle className="text-rose-400" size={24} />
          <div>
            <p className="text-sm font-bold text-rose-400">?∞Ïù¥??Î°úÎìú ?§Ìå®</p>
        ...
        <div className="grid grid-cols-1 gap-4">
            ...
            <div key={config.id} className={`admin-card p-5 border-l-4 transition-all hover:bg-zinc-800/30 ${config.is_active ? 'border-l-admin-brand' : 'border-l-zinc-800 opacity-60'}`}>
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3 self-end lg:self-center">
                  ...
                  <button
                    onClick={() => {
                      if (!window.confirm("???§Ï†ï????†ú?†Íπå??")) return;
          ...
          {(data ?? []).length === 0 && (
            <div className="admin-card p-20 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div className="space-y-1">
                <p className="text-lg font-bold text-white">?±Î°ù??Î£∞Î†õ???ÜÏäµ?àÎã§</p>
                <p className="text-sm text-zinc-500">?àÎ°ú??Î£∞Î†õ ?§Ï†ï??Ï∂îÍ??òÏó¨ ?¥ÏòÅ???úÏûë?òÏÑ∏??</p>
              </div>
              <button
                onClick={openCreate}
          ...
          <div className="relative w-full max-w-5xl bg-zinc-900 rounded-[32px] border border-zinc-800 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-0.5">
                  <h3 className="text-xl font-black text-white tracking-tight">
                    {editing ? "Î£∞Î†õ ?§Ï†ï ?òÏ†ï" : "??Î£∞Î†õ ?§Ï†ï Ï∂îÍ?"}
                  </h3>
              ...
              <form id="roulette-form" onSubmit={onSubmit} className="space-y-8">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-admin-brand">
                    <Info size={16} />
                    <h4 className="text-sm font-black uppercase tracking-widest">Í∏∞Î≥∏ ?ïÎ≥¥ (Basic Information)</h4>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">Î£∞Î†õ Í¥ÄÎ¶¨Ïûê Î™ÖÏπ≠</label>
                      <input
                        type="text"
                        ...
                        placeholder="?? Í∏∞Í∞Ñ?úÏ†ï ?§Ïù¥??Î£∞Î†õ"
                        {...form.register("name")}
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">?åÎ™® ?∞Ïºì ?Ä??/label>
                      <select
                      ...
                      >
                        <option value="ROULETTE_COIN">ÏΩîÏù∏ (ROULETTE_COIN)</option>
                        <option value="TRIAL_TOKEN">Ï≤¥ÌóòÍ∂?(TRIAL_TOKEN)</option>
                        <option value="GOLD_KEY">Í≥®Îìú??(GOLD_KEY)</option>
                        <option value="DIAMOND_KEY">?§Ïù¥?ÑÌÇ§ (DIAMOND_KEY)</option>
                      </select>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">?ÄÍ≤??∏Í∑∏Î®ºÌä∏ (Grade)</label>
                      <select
                        className="w-full h-12 bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 text-sm text-white focus:border-admin-brand outline-none transition-colors appearance-none"
                        {...form.register("grade")}
                      >
                        <option value="COMMON">?ºÎ∞ò (COMMON)</option>
                        <option value="NEW">?†Í∑ú ?†Ï? (NEW)</option>
                        <option value="WHALE">Í≥†Ïï° ?†Ï? (WHALE)</option>
                      </select>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">?ºÏùº ?§Ì? ?úÌïú (0=Î¨¥Ï†ú??</label>
                      <input
                        type="number"
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-admin-brand">
                      <LayoutGrid size={16} />
                      <h4 className="text-sm font-black uppercase tracking-widest">?∏Í∑∏Î®ºÌä∏ ?§Ï†ï (Slots & Probability)</h4>
                      <button
                        type="button"
                ...
                <div className="pt-6 border-t border-zinc-800">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-admin-brand">
                      <RefreshCw size={16} /> Í≤ÄÏ¶??úÎ??àÏù¥??(Verification)
                    </h4>
                    <button
                      type="button"
                  ...
                  {simResults ? (
                    ...
                    <div className="p-6 bg-zinc-800/30 border border-zinc-800 border-dashed rounded-2xl text-center">
                      <p className="text-xs text-zinc-500 font-medium">??Î≤ÑÌäº???åÎü¨ ?ÑÏû¨ ?§Ï†ï???ïÎ•†?ÄÎ°?1,000???úÎ??àÏù¥?òÏùÑ ?§Ìñâ?òÏó¨ Í≤∞Í≥ºÎ•?Í≤ÄÏ¶ùÌï©?àÎã§.</p>
                    </div>
                  )}
                </div>
              </form>
            ...
            <div className="p-6 border-t border-zinc-800 bg-zinc-900/50 flex flex-col sm:flex-row gap-3 justify-end items-center">
              <div className="flex-1 text-xs text-zinc-500 hidden sm:block">
                <span className="font-bold text-zinc-400">NOTE:</span> Î≥ÄÍ≤ΩÎêú ?§Ï†ï?Ä ?Ä??Ï¶âÏãú ?úÏä§?úÏóê Î∞òÏòÅ?©Îãà??
              </div>
              <button
                onClick={closeModal}
              ...
              >
                {mutation.isPending ? "?Ä??Ï§?.." : "?§Ï†ï ?Ä??(Save Config)"}
              </button>
            </div>
          </div>
        </div>
