      ...
      <main className="flex-1 w-full max-w-[1650px] mx-auto p-8 lg:p-14 space-y-10">
        ...
        <section className="space-y-6">
          <h1 className="text-3xl font-bold text-admin-text-base tracking-tight uppercase">
            ?¥ÏòÅ ÏßÄ???îÏïΩ <span className="text-admin-brand/40">Dashboard</span>
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Card 1: Active Users */}
            <div
              onClick={() => handleCardClick("Í∏àÏùº ?úÏÑ±", "today_active")}
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-1">
                  <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Í∏àÏùº ?úÏÑ±</span>
            ...
            {/* Card 2: Revenue */}
            <div
              onClick={() => handleCardClick("Í∏àÏùº ?ÖÍ∏à", "today_deposit")}
              className="flex flex-col p-6 gap-4 bg-zinc-800/60 backdrop-blur-xl border border-white/5 rounded-xl shadow-2xl relative overflow-hidden group cursor-pointer hover:bg-zinc-800/80 transition-all"
            >
              ...
              <div className="flex justify-between items-start relative z-10">
                <div className="flex flex-col gap-1">
                  <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Í∏àÏùº ?ÖÍ∏à (Est)</span>
            ...
            {/* Card 3: Churn Risk */}
            <div
              onClick={() => handleCardClick("?¥ÌÉà Î¶¨Ïä§??, "churn_risk")}
              className="flex flex-col p-6 gap-4 bg-zinc-800/60 backdrop-blur-xl border border-white/5 rounded-xl shadow-2xl relative overflow-hidden group cursor-pointer hover:bg-zinc-800/80 transition-all"
            >
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-1">
                  <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">?¥ÌÉà Î¶¨Ïä§??/span>
            ...
            {/* Card 4: Game Play */}
            <div
              onClick={() => handleCardClick("Í≤åÏûÑ ?åÎ†à??, "today_game_plays")}
              className="flex flex-col p-6 gap-4 bg-zinc-800/60 backdrop-blur-xl border border-white/5 rounded-xl shadow-2xl relative overflow-hidden group cursor-pointer hover:bg-zinc-800/80 transition-all"
            >
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-1">
                  <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Í≤åÏûÑ ?åÎ†à??/span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-4xl max-h-[80vh] flex flex-col rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              <header className="flex items-center justify-between border-b border-zinc-800 px-6 py-4">
                <div>
                  <h3 className="text-lg font-bold text-white">{selectedMetric?.title} ?ÅÏÑ∏ ?¥Ïó≠</h3>
                  <p className="text-sm text-zinc-500">
                    {metricDetails?.length ?? 0}Í±¥Ïùò ?¥Ïó≠??Ï°∞Ìöå?òÏóà?µÎãà??
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                {isDetailsLoading ? (
                  <div className="flex flex-col items-center justify-center py-20 text-zinc-500">
                    ...
                    <p>?ÅÏÑ∏ ?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§?..</p>
                  </div>
                ) : isDetailsError ? (
                  <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-red-200">
                    <div className="font-semibold">Ï°∞Ìöå ?§Ìå®</div>
                    <div className="mt-1 text-sm opacity-90">
                      {String((detailsError as any)?.message ?? "?úÎ≤Ñ ?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.")}
                    </div>
                  </div>
                ) : (
                  <>
