        ...
        <div className="space-y-6">
            ...
            <div className="flex items-stretch gap-4 overflow-x-auto pb-2 custom-scrollbar">
                <MetricCard
                    ...
                    subValue={stats?.expiring_soon_24h > 0 ? `ÎßåÎ£å ?ÑÎ∞ï ${stats.expiring_soon_24h}Î™? : "?àÏ†ï??}
                    icon={TrendingUp}
                ...
                <MetricCard
                    ...
                    subValue={stats ? `?†Í∏à ${formatCurrency(stats.total_locked || 0)}` : "-"}
                    icon={Users}
                ...
                >
                    <div className="flex flex-col gap-1 z-10 relative">
                        ...
                        <div className={`text-lg font-black tracking-tight ${goldenHourEnabled ? 'text-yellow-400' : 'text-zinc-500'}`}>
                            {goldenHourEnabled ? "Î∂Ä?§ÌÑ∞ Í∞Ä?ôÏ§ë" : "?ÄÍ∏?OFF)"}
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-admin-card w-full max-w-4xl rounded-2xl border border-admin-border shadow-2xl max-h-[80vh] flex flex-col">
                        <div className="flex justify-between items-center p-6 border-b border-admin-border">
                            <h3 className="text-xl font-bold text-white">{detailModal.title} - ?ÅÏÑ∏ ?¥Ïó≠</h3>
                            <button
                                onClick={() => setDetailModal(null)}
                                className="text-admin-text-muted hover:text-white transition-colors"
                                aria-label="?ÅÏÑ∏ Î™®Îã¨ ?´Í∏∞"
                                title="?´Í∏∞"
                            >
                                <X className="h-5 w-5" />
                            </button>
                        ...
                        <div className="flex-1 overflow-auto p-6">
                            {loadingDetail ? (
                                <div className="text-center py-12 text-admin-text-secondary">Î°úÎî© Ï§?..</div>
                            ) : !detailData || !detailData.items || detailData.items.length === 0 ? (
                                <div className="text-center py-12 text-admin-text-muted">?∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§.</div>
                            ) : (
                                    ...
                                    {detailData.items.map((item: any, idx: number) => (
                                        <div key={idx} className="bg-admin-sidebar/50 border border-admin-border rounded-xl p-4 hover:bg-admin-hover transition-colors">
                                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                                <div>
                                                    <div className="text-admin-meta text-admin-text-secondary mb-1">?¨Ïö©??/div>
                                                ...
                                                <div>
                                                    <div className="text-admin-meta text-admin-text-secondary mb-1">Í∏àÏï°</div>
                                                    <div className="font-black text-admin-brand">{formatCurrency(item.amount)}</div>
                                                    {item.count && item.count > 1 && <div className="text-xs text-admin-text-muted">{item.count}Í±?/div>}
                                                </div>
                                                <div>
                                                    <div className="text-admin-meta text-admin-text-secondary mb-1">?ºÏãú</div>
                                                    <div className="text-admin-body text-admin-text-primary">
                                                        {item.timestamp ? dayjs(item.timestamp).format("YYYY-MM-DD HH:mm") : "-"}
                                                    </div>
                                                </div>
                                                <div>
                                                    <div className="text-admin-meta text-admin-text-secondary mb-1">Î©îÌ?</div>
            ...
            <ConfirmModal
                ...
                title={goldenHourEnabled ? "Í≥®Îì† ?ÑÏõå Ï¢ÖÎ£å" : "Í≥®Îì† ?ÑÏõå ?úÏûë"}
                message={goldenHourEnabled
                    ? `?ÑÏû¨ ?ÅÏö© Ï§ëÏù∏ ${config?.config_json?.golden_hour_config?.multiplier || 1.5}Î∞??ÅÎ¶Ω Î∂Ä?§ÌÑ∞Î•?Ï¢ÖÎ£å?òÏãúÍ≤†Ïäµ?àÍπå?`
                    : `ÏßÄÍ∏àÎ???Î™®Îì† ?†Ï???Í∏àÍ≥† ?ÅÎ¶ΩÎ•†ÏùÑ ${config?.config_json?.golden_hour_config?.multiplier || 1.5}Î∞∞Î°ú ?ÅÌñ•?©Îãà?? ÏßÑÌñâ?òÏãúÍ≤†Ïäµ?àÍπå?`}
                confirmText={goldenHourEnabled ? "Ï¢ÖÎ£å?òÍ∏∞" : "?úÏûë?òÍ∏∞ (Fire)"}
