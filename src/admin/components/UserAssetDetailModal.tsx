...
const UserAssetDetailModal: React.FC<UserAssetDetailModalProps> = ({
  ...
  const handleVaultUpdate = async () => {
    ...
    if (
      !window.confirm(`?ïÎßê ID:${userId} ?†Ï???Í∏àÍ≥† ?îÏï°???òÏ†ï?òÏãúÍ≤†Ïäµ?àÍπå?`)
    ) {
      return;
    }
    ...
    try {
      ...
      addToast("Í∏àÍ≥† ?îÏï°???òÏ†ï?òÏóà?µÎãà??", "success");
      vaultQuery.refetch();
      ...
      <div className="w-full max-w-4xl bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex-none px-6 border-b border-zinc-800 bg-zinc-900/30">
          <div className="flex gap-6">
            {[
              { id: "summary", label: "?êÏÇ∞ ?îÏïΩ", icon: Wallet },
              { id: "playLogs", label: "?åÎ†à??Î°úÍ∑∏", icon: ScrollText },
              { id: "ledger", label: "ÏßÄÍ∞??êÏû•", icon: History },
              { id: "inventory", label: "?∏Î≤§?†Î¶¨", icon: Package },
              { id: "vault", label: "Í∏àÍ≥† Í¥ÄÎ¶?, icon: Lock },
            ].map((tab) => (
              <button
                key={tab.id}
        ...
        <div className="flex-1 overflow-auto p-6 bg-black/20 custom-scrollbar">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {userSummary ? (
                  ...
                  <div className="text-zinc-500">?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§?..</div>
                )}
              </div>
              ...
              {playLogsQuery.isLoading ? (
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="text-xs text-zinc-500 uppercase border-b border-zinc-800">
                    <tr>
                      <th className="px-4 py-3">?úÍ∞Ñ</th>
                      <th className="px-4 py-3">Í≤åÏûÑ</th>
                      <th className="px-4 py-3 text-right">Î≥¥ÏÉÅ</th>
                    </tr>
                  </thead>
              ...
              {ledgerQuery.isLoading ? (
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="text-xs text-zinc-500 uppercase border-b border-zinc-800">
                    <tr>
                      <th className="px-4 py-3">?úÍ∞Ñ</th>
                      <th className="px-4 py-3">?∞Ïºì</th>
                      <th className="px-4 py-3 text-right">Î≥Ä??/th>
                      <th className="px-4 py-3 text-right">?îÏï°</th>
                      <th className="px-4 py-3">?¨Ïú†</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-800">
          ...
          {activeTab === "inventory" && (
            <div className="space-y-8">
              ...
              <div>
                <h4 className="text-sm font-bold text-zinc-400 mb-3 uppercase tracking-wider border-b border-zinc-800 pb-2">
                  ?∏Î≤§?†Î¶¨ Í∏∞Î°ù (Recent 100)
                </h4>
                <table className="w-full text-left border-collapse">
                  <thead className="text-xs text-zinc-500 uppercase border-b border-zinc-800">
                    <tr>
                      <th className="px-4 py-3">?úÍ∞Ñ</th>
                      <th className="px-4 py-3">?ÑÏù¥??/th>
                      <th className="px-4 py-3 text-right">Î≥Ä??/th>
                      <th className="px-4 py-3 text-right">?îÏó¨</th>
                      <th className="px-4 py-3">?¨Ïú†</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-800">
                  ...
                  <div className="bg-red-900/5 border border-red-500/20 rounded-2xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-red-500/20 bg-red-900/10 flex items-center justify-between">
                      <h3 className="text-sm font-black text-white flex items-center gap-2">
                        ...
                        Í∏àÍ≥† ?îÏï° Í∞ïÏ†ú ?òÏ†ï (Force Update)
                      </h3>
                    </div>
                    ...
                    <div className="p-6 space-y-6">
                      ...
                      <div className="bg-rose-500/5 border border-rose-500/20 rounded-xl p-4 flex items-start gap-4">
                        ...
                        <div className="text-[11px] text-zinc-400 leading-relaxed">
                          ???ëÏóÖ?Ä ?§ÏãúÍ∞??êÏû•??Ï¶âÏãú Î∞òÏòÅ?òÎ©∞ Î≥µÍµ¨????
                          ?ÜÏäµ?àÎã§. ?†Ï§ë?òÍ≤å ÏßÑÌñâ?¥Ï£º?∏Ïöî.
                        </div>
                      </div>
