...
const UserInventoryModal: React.FC<UserInventoryModalProps> = ({ memberId, isOpen, onClose, nickname }) => {
  ...
  const adjustMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      alert("?∏Î≤§?†Î¶¨ Ï°∞Ï†ï???ÑÎ£å?òÏóà?µÎãà??");
      setDelta(0);
      setNote("");
      setActiveTab("items");
    },
    onError: (err: any) => {
      alert(`Ï°∞Ï†ï ?§Ìå®: ${err.message}`);
    }
  });
      ...
      <div className="admin-card w-full max-w-2xl max-h-[90vh] flex flex-col shadow-admin-glow border-admin-brand/20">
        ...
        <div className="p-6 border-b border-admin-border flex items-center justify-between bg-admin-sidebar/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-admin-subtitle text-admin-text-primary">?åÏõê ?∏Î≤§?†Î¶¨ Í¥ÄÎ¶?/h2>
              <p className="text-admin-meta text-admin-text-muted">{nickname || memberId} ?åÏõê??Î≥¥Ïú† ?ÑÏù¥???úÏñ¥</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-admin-bg/30">
          {invQuery.isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 text-admin-text-muted gap-3">
              ...
              <p className="text-admin-meta">?∏Î≤§?†Î¶¨ ?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§ëÏûÖ?àÎã§...</p>
            </div>
          ) : invQuery.isError ? (
            <div className="py-20 text-center text-admin-danger">
              ?∏Î≤§?†Î¶¨ Ï°∞Ìöå???§Ìå®?àÏäµ?àÎã§.
            </div>
          ) : activeTab === "items" ? (
            <div className="space-y-8">
              {/* Items List */}
              <div className="space-y-3">
                <h3 className="text-admin-meta font-bold text-admin-text-secondary uppercase tracking-wider px-1">?ÑÏû¨ Î≥¥Ïú† ?ÑÏù¥??/h3>
                <div className="grid grid-cols-2 gap-3">
                  {invQuery.data?.items?.length ? (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-admin-text-muted">
                              ÏµúÍ∑º ?ÖÎç∞?¥Ìä∏: {new Date(item.updated_at).toLocaleString("ko-KR", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false })}
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="col-span-2 py-8 text-center border-2 border-dashed border-admin-border rounded-xl opacity-40">
                      <p className="text-admin-meta">Î≥¥Ïú† Ï§ëÏù∏ ?ÑÏù¥?úÏù¥ ?ÜÏäµ?àÎã§.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Ledger History */}
              <div className="space-y-3">
                <h3 className="text-admin-meta font-bold text-admin-text-secondary uppercase tracking-wider px-1">ÏµúÍ∑º Î≥Ä???¥Î†•</h3>
                <div className="space-y-2">
                  {invQuery.data?.ledger?.length ? (
                    ...
                    <div className="py-8 text-center border-2 border-dashed border-admin-border rounded-xl opacity-40">
                      <p className="text-admin-meta">Î≥Ä???¥Î†•???ÜÏäµ?àÎã§.</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="admin-label">?ÑÏù¥??ÏΩîÎìú</label>
                  <select
                    ...
                    aria-label="?ÑÏù¥??ÏΩîÎìú ?†ÌÉù"
                  >
                    <option value="" disabled>
                      ?ÑÏù¥???†ÌÉù
                    </option>
                    {ownedItemOptions.length > 0 && (
                      <optgroup label="Î≥¥Ïú† ?ÑÏù¥??>
                    ...
                    <option value="__CUSTOM__">ÏßÅÏ†ë ?ÖÎ†•</option>
                  </select>
                  {itemTypeMode === "custom" && (
                    <input
                      type="text"
                  ...
                  {rewardTypesQuery.isError && (
                    <p className="text-[11px] text-admin-danger">?ÑÏù¥??Î™©Î°ù??Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà?? ÏßÅÏ†ë ?ÖÎ†•???¨Ïö©??Ï£ºÏÑ∏??</p>
                  )}
                </div>
                <div className="space-y-2">
                  <label className="admin-label">Î≥Ä???òÎüâ (Delta)</label>
                  <input
                    type="number"
                    value={delta}
              ...
              <div className="space-y-2">
                <label className="admin-label">Ï°∞Ï†ï Î©îÎ™® (?¥ÏòÅ Í∏∞Î°ù??</label>
                <textarea
                  value={note}
                  ...
                  placeholder="CS ?Ä???¨Ïú† ?±ÏùÑ ?ÖÎ†•?òÏÑ∏??"
                />
              </div>
              ...
              <div className="p-4 rounded-xl bg-admin-warning/5 border border-admin-warning/20 flex items-start gap-3">
                ...
                <div className="text-admin-meta text-admin-text-secondary leading-relaxed">
                  <p className="font-bold text-admin-warning mb-1">?∏Î≤§?†Î¶¨ ÏßÅÏ†ë Ï°∞Ï†ï Ï£ºÏùò?¨Ìï≠</p>
                  DIAMOND?Ä Í∞ôÏ? ?µÏã¨ ?¨Ìôî???úÏ¶å ?úÏ†ï ?ÑÏù¥??Ï°∞Ï†ï ?? Î∞òÎìú???¨Ï†Ñ???πÏù∏???åÌÅ¨?åÎ°ú?∞Î? ?∞ÎùºÏ£ºÏÑ∏?? Î™®Îì† Ï°∞Ï†ï ?¥Ïó≠?Ä ?¥ÏòÅ ?∏Îûú??Öò ?ÑÏ†Å??Í∏∞Î°ù?©Îãà??
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-6 border-t border-admin-border bg-admin-sidebar/50 flex justify-end gap-3">
          ...
          {activeTab === "adjust" && (
            <button
              onClick={() => {
                if (!itemType.trim()) return alert("?ÑÏù¥??ÏΩîÎìúÎ•??ÖÎ†•?¥Ï£º?∏Ïöî.");
                if (delta === 0) return alert("Î≥Ä???òÎüâ???ÖÎ†•?¥Ï£º?∏Ïöî.");
