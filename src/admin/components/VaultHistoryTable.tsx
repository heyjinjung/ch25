...
const VaultHistoryTable: React.FC<VaultHistoryTableProps> = ({
  user,
  onClose,
}) => {
  const {
          ...
          <div className="space-y-1">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              <span className="text-sm font-normal text-zinc-400">
                (User:{" "}
                {user.nickname || user.telegram_username || user.external_id})
              </span>
            </h2>
            <p className="text-xs text-zinc-500">
              최근 100건의 금고 변???�역??조회?�니??
            </p>
            {vaultState && (
              <div className="flex items-center gap-4 mt-2 pt-2 border-t border-zinc-800/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-emerald-400">
                    ?�재 ?�적 금고??{" "}
                    {vaultState.locked_balance.toLocaleString()}??
                  </span>
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-3 text-sm font-medium text-white">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded textxs font-medium ${
                            event.earn_type === "GAME_PLAY"
                              ? "bg-emerald-500/10 text-emerald-400"
                              : event.earn_type === "MISSION_REWARD"
                                ? "bg-amber-500/10 text-amber-400"
                                : event.earn_type === "TRIAL_PAYOUT"
                      ...
                      <td
                        className="px-6 py-3 text-xs text-zinc-500 font-mono max-w-[200px] truncate"
                        title={event.earn_event_id}
                      >
                        {event.earn_event_id}
                      </td>
                      <td
                        className={`px-6 py-3 text-sm font-bold text-right font-mono ${
                          isPositive
                            ? "text-emerald-400"
                            : isNegative
                              ? "text-red-400"
