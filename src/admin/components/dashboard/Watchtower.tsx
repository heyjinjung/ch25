...
const Watchtower: React.FC = () => {
  ...
  const getThreatBg = (level: string) => {
    switch (level) {
      case "CRITICAL":
        return "bg-admin-danger/10 border-admin-danger";
      case "ELEVATED":
        return "bg-admin-warning/10 border-admin-warning";
      default:
        return "bg-admin-accent/10 border-admin-accent";
    }
  };
      ...
      <div className="p-6 border-b border-admin-border flex items-center justify-between relative z-10">
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <div
              className={`absolute inset-0 ${getThreatColor(threatLevel)} animate-ping opacity-20`}
            ></div>
          </div>
        ...
        <div
          className={`px-3 py-1 rounded-full border text-xs font-black tracking-wider flex items-center gap-2 ${getThreatBg(threatLevel)} ${getThreatColor(threatLevel)}`}
        >
