...
const QuickActionsBar: React.FC = () => {
    ...
    const contextualActions: QuickAction[] = [
        ...(riskCount > 0 ? [{
            id: "risk-users",
            label: "이탈 위험",
            description: `${riskCount}명 리스크 감지`,
            to: "/admin/marketing",
        ...
        ...(streakRiskCount > 0 ? [{
            id: "streak-risk",
            label: "스트릭 위험",
            description: `${streakRiskCount}명 이탈 임박`,
        ...
        <div className="admin-card-premium p-6">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-admin-text-primary">빠른 작업</h3>
                </div>
                <span className="text-[10px] text-admin-text-muted font-mono">
                    {contextualActions.length > 0 ? `${contextualActions.length}건 대기` : "정상"}
                </span>
            </div>
            ...
            {contextualActions.length > 0 && (
                <div className="mb-4 space-y-2">
                    <p className="text-[10px] font-bold text-admin-warning uppercase tracking-wider flex items-center gap-1">
                        <Clock className="h-3 w-3" /> 즉시 조치 필요
                    </p>
