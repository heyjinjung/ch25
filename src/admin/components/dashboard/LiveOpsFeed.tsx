  ...
  const toSummary = (log: {
    ...
    if (log.action_code === "OFFER_PERSONALIZED_TRACKED") {
      ...
      if (trigger === "PREDICTIVE_REENGAGEMENT") {
        ...
        const churn =
          meta.churn_probability !== undefined
            ? `(${Number(meta.churn_probability).toFixed(2)})`
            : "";
    ...
    <div className="bg-zinc-800/60 backdrop-blur-xl border border-white/5 rounded-xl flex flex-col h-full overflow-hidden relative group">
      ...
      <div className="flex items-center justify-between p-6 border-b border-white/5 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-[24px] font-light text-zinc-200 tracking-tight">
            ?�시�??�영 ?�드
          </h2>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-900/50 border border-white/5">
          ...
          <span className="text-[12px] text-zinc-500">라이브</span>
        </div>
      </div>
                ...
                <div className="mt-1 flex-shrink-0">
                  <div
                    className={`p-2 rounded-lg bg-zinc-900 border border-white/5 ${
                      log.category === OpsLogCategoryValues.SECURITY
                        ? "text-rose-400"
                        : log.category === OpsLogCategoryValues.ECONOMY
                          ? "text-emerald-400"
                          : "text-indigo-400"
                    }`}
                  >
