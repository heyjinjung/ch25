...
const SeasonOpsBoard: React.FC = () => {
    ...
    if (!activeSeason) {
        return (
            <div className="admin-card-premium p-6 flex flex-col items-center justify-center min-h-[300px] text-center gap-4">
                ...
                <div>
                    <h3 className="text-admin-subtitle text-admin-text-primary">ÏßÑÌñâ Ï§ëÏù∏ ?úÏ¶å ?ÜÏùå</h3>
                    <p className="text-admin-body text-admin-text-secondary mt-1">?àÎ°ú???úÏ¶å???àÏïΩ?òÍ±∞???úÏÑ±?îÌï¥Ï£ºÏÑ∏??</p>
                </div>
                <button className="btn-admin-primary mt-2">
                    ?úÏ¶å Í¥ÄÎ¶?Î∞îÎ°úÍ∞ÄÍ∏?
                </button>
            </div>
        );
    }
    ...
    const totalRewardsValue = levels.reduce((acc, lvl) => acc + lvl.reward_amount, 0);
            ...
            <div className="flex flex-col gap-8 relative z-10 w-full">
                ...
                <div className="space-y-3 w-full max-w-[920px]">
                    <div className="h-3 bg-zinc-900 border border-white/5 rounded-full overflow-hidden relative">
                        ...
                        >
                            <div className="absolute right-0 top-0 bottom-0 w-1 bg-white/50 animate-pulse shadow-[0_0_10px_rgba(255,255,255,0.5)]"></div>
                        </div>
                    </div>
                    <div className="flex justify-between text-[10px] font-light text-zinc-500">
                        <span>?úÏûë {dayjs(activeSeason.start_date).format("YYYY-MM-DD")}</span>
                        <span>Ï¢ÖÎ£å {dayjs(activeSeason.end_date).format("YYYY-MM-DD")}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 mt-8 relative z-10 border-t border-white/5 pt-8">
                ...
                <div className="flex flex-col gap-2 p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">?ÑÏ≤¥ Î≥¥ÏÉÅ</span>
                    </div>
                    <span className="text-2xl font-black text-zinc-200">
                        {totalRewardsValue.toLocaleString()}
                    </span>
                    <span className="text-[10px] text-zinc-600">Ï¥ùÌï©</span>
                </div>
                ...
                <div className="flex flex-col gap-2 p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Ï∞∏Ïó¨</span>
                    </div>
                    <span className="text-2xl font-black text-zinc-200">--</span>
                    <span className="text-[10px] text-zinc-600">?úÏÑ± ?åÎ†à?¥Ïñ¥</span>
                </div>
            </div>
        </div>
    );
