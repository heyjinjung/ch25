...
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Dialog */}
      <div
        className={`relative w-full max-w-md bg-admin-card backdrop-blur-xl border border-admin-border rounded-admin-xl shadow-2xl overflow-hidden animate-scaleIn`}
      >
        <div className="p-6">
          <div className="flex items-start gap-4">
            <div
              className={`p-3 rounded-full flex-shrink-0 ${type === "danger" ? "bg-admin-danger/10 text-admin-danger" : "bg-admin-brand/10 text-admin-brand"}`}
            >
              {type === "danger" ? (
        ...
        <div className="px-6 py-4 bg-admin-sidebar/50 border-t border-admin-border flex justify-end gap-3">
          ...
          <button
            onClick={onConfirm}
            className={`px-6 py-2 rounded-admin-lg text-white font-bold text-sm shadow-lg transition-all active:scale-95 ${
              type === "danger"
