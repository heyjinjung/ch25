...
const TOKEN_GROUPS: Array<{ label: string; keys: GameTokenType[] }> = [
  ...
  { label: "Í∏àÍ≥† ?¥Ïá†", keys: ["GOLD_KEY", "DIAMOND_KEY"] },
  { label: "?¨Ìôî", keys: ["DIAMOND"] },
  { label: "?ºÏ¶ê", keys: ["PUZZLE_C1", "PUZZLE_C2", "PUZZLE_J", "PUZZLE_M"] },
];

const TOKEN_LABELS: Partial<Record<GameTokenType, string>> = {
  ROULETTE_COIN: "Î£∞Î†õ ÏΩîÏù∏",
  DICE_TOKEN: "Ï£ºÏÇ¨???†ÌÅ∞",
  LOTTERY_TICKET: "Î≥µÍ∂å ?∞Ïºì",
  TRIAL_TOKEN: "Ï≤¥Ìóò ?†ÌÅ∞",
  GOLD_KEY: "?©Í∏à ?¥Ïá†",
  DIAMOND_KEY: "?§Ïù¥????,
  DIAMOND: "?§Ïù¥??,
  PUZZLE_C1: "?ºÏ¶ê C1",
...
const UserGameTokenModal: React.FC<UserGameTokenModalProps> = ({
  ...
  const actionLabel = actionMode === "grant" ? "ÏßÄÍ∏? : "?åÏàò";
  ...
  const grantMutation = useMutation({
    ...
    onSuccess: () => {
      addToast("ÏßÄÍ∏âÏù¥ ?ÑÎ£å?òÏóà?µÎãà??", "success");
      invalidateAll();
    ...
    onError: (err: any) => {
      addToast(err.response?.data?.detail || "ÏßÄÍ∏??§Ìå®", "error");
    },
  });
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: () => {
      addToast("?åÏàòÍ∞Ä ?ÑÎ£å?òÏóà?µÎãà??", "success");
      invalidateAll();
      walletsQuery.refetch();
      inventoryQuery.refetch();
      ledgerQuery.refetch();
    ...
    onError: (err: any) => {
      addToast(err.response?.data?.detail || "?åÏàò ?§Ìå®", "error");
    },
  });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-admin-subtitle text-admin-text-primary">
                Í≤åÏûÑ ÏΩîÎìú/?†ÌÅ∞ ?úÏñ¥
              </h2>
              <p className="text-admin-meta text-admin-text-muted">
                {nickname || memberId} ?åÏõê ?êÏÇ∞ Í¥ÄÎ¶?              </p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          {!isNumericId && (
            <div className="mb-6 rounded-xl border border-admin-danger/30 bg-admin-danger/10 p-4 text-admin-danger text-sm">
              ?´Ïûê ?åÏõê IDÎ•??ïÏù∏?????ÜÏäµ?àÎã§. ?åÏõê ?ùÎ≥Ñ?êÎ? ?§Ïãú ?ïÏù∏??              Ï£ºÏÑ∏??
            </div>
          )}
          ...
          {activeTab === "grant" ? (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-admin-text-muted">
                    ?ÑÏû¨ Î≥¥Ïú†: {getTokenBalance(tokenType).toLocaleString()}
                  </p>
                </div>
                <div className="space-y-2">
                  <label className="admin-label">?òÎüâ</label>
                  <input
                    type="number"
                    value={amount}
              ...
              <div className="space-y-2">
                <label className="admin-label">ÏßÄÍ∏??¨Ïú†</label>
                <textarea
                  value={reason}
                  ...
                  placeholder="ÏßÄÍ∏??¨Ïú†Î•??ÖÎ†•?òÏÑ∏??(?? ?¥Î≤§??Î≥¥ÏÉÅ, Î≤ÑÍ∑∏ Î≥¥ÏÉÅ ??"
                />
              </div>
              ...
              <div className="p-4 rounded-xl bg-admin-brand/5 border border-admin-brand/10 flex items-start gap-3">
                ...
                <p className="text-admin-meta text-admin-text-secondary leading-relaxed">
                  ?†ÌÅ∞??{actionLabel}?òÎ©¥ Ï¶âÏãú ?åÏõê??ÏßÄÍ∞??∏Î≤§?†Î¶¨??Î∞òÏòÅ?òÎ©∞,
                  ?¥ÏòÅ ?∏Îûú??Öò ?ÑÏ†Å???ÅÍµ¨??Í∏∞Î°ù?©Îãà?? ?†Ï§ë?òÍ≤å ?ëÏóÖ??                  Ï£ºÏÑ∏??
                </p>
              </div>
            </div>
          ) : (
              ...
              <div className="space-y-3">
                {ledgerQuery.isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-admin-text-muted border-2 border-dashed border-admin-border rounded-2xl">
                    ...
                    <p className="text-admin-meta">Î≥ÄÍ≤??¥Î†•???ÜÏäµ?àÎã§.</p>
                  </div>
                ) : (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-admin-meta text-admin-text-primary mb-1">
                          ?îÏï°: {entry.balance_after.toLocaleString()}
                        </p>
