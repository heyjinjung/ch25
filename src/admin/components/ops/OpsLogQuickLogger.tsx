...
const CATEGORY_LABEL: Record<OpsLogCategory, string> = {
  ...
  EXPERIMENT: "?§Ìóò",
  ANALYTICS: "Î∂ÑÏÑù",
  GAME_PLAY: "Í≤åÏûÑ ?åÎ†à??,
  ECONOMY: "Í≤ΩÏ†ú",
  SECURITY: "Î≥¥Ïïà",
  USER_MANAGEMENT: "?åÏõê Í¥ÄÎ¶?,
};
...
const OpsLogQuickLogger: React.FC<OpsLogQuickLoggerProps> = ({ variant = "full", defaultDate, listFilters }) => {
  ...
  const submit = async () => {
    if (!date) {
      addToast("?†Ïßú???ÑÏàò?ÖÎãà??", "error");
      return;
    }
    if (!actionCode.trim()) {
      addToast("action_code???ÑÏàò?ÖÎãà??", "error");
      return;
    }
    if (!parsedMeta.ok) {
      addToast(`meta_data JSON ?§Î•ò: ${parsedMeta.error}`, "error");
      return;
    }
    ...
    if (piiHits.length > 0) {
      addToast(`PII ?òÏã¨ ?®ÌÑ¥ Í∞êÏ?(${piiHits[0].type}): Í∏∞Î°ù Ï∞®Îã®`, "error");
      return;
    }
    ...
    try {
      ...
      addToast(`Í∏∞Î°ù ?ÑÎ£å: #${created.id}`, "success");
      ...
      addToast(err?.response?.data?.detail || err?.message || "Í∏∞Î°ù ?§Ìå®", "error");
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-admin-text-base">
              ?¥ÏòÅ ??Î°úÍ±∞ <span className="text-admin-brand/50">Quick Logger</span>
            </h3>
            <p className="mt-0.5 text-[10px] font-bold text-admin-text-muted">Í¥ÄÎ¶¨Ïûê ?òÎèô ?ÖÎ†• Ï¶âÍ∞Å Ï∂îÏ†Å Î∞?Í∏∞Î°ù</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 rounded-full border border-admin-border bg-admin-bg px-3 py-1">
            ...
            <span className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest">
              {createMutation.isPending ? "?ôÍ∏∞??Ï§? : "?ÄÍ∏?}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="relative grid grid-cols-1 gap-6 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="space-y-4">
            <div className="group/input">
              <label className="mb-1.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-admin-text-muted group-focus-within/input:text-admin-brand transition-colors">
                <span>Î∞úÏÉù ?†Ïßú</span>
                <div className="h-px flex-1 bg-admin-border" />
              </label>
              <input
                type="date"
                aria-label="Î∞úÏÉù ?†Ïßú"
                title="Î∞úÏÉù ?†Ïßú"
                value={date}
                onChange={(e) => {
            ...
            <div className="group/input">
              <label className="mb-1.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-admin-text-muted group-focus-within/input:text-admin-brand transition-colors">
                <span>Î°úÍ∑∏ Î∂ÑÎ•ò</span>
                <div className="h-px flex-1 bg-admin-border" />
              </label>
              <select
                aria-label="Î°úÍ∑∏ Î∂ÑÎ•ò"
                title="Î°úÍ∑∏ Î∂ÑÎ•ò"
                value={category}
                onChange={(e) => {
          ...
          <div className="space-y-4">
            <div className="group/input">
              <label className="mb-1.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-admin-text-muted group-focus-within/input:text-admin-brand transition-colors">
                <span>?°ÏÖò ÏΩîÎìú</span>
                <div className="h-px flex-1 bg-admin-border" />
              </label>
              <input
                aria-label="?°ÏÖò ÏΩîÎìú"
                title="?°ÏÖò ÏΩîÎìú"
                value={actionCode}
                onChange={(e) => {
          ...
          <div className="group/input">
            ...
            <div className="relative">
              <input
                ...
                placeholder="ÎπÑÏõå?êÎ©¥ ?êÎèô ?ùÏÑ±"
          ...
          {!isCompact ? (
            ...
            <div className="rounded-lg border border-admin-border border-dashed bg-admin-bg/30 p-3">
              <p className="text-[10px] font-bold text-admin-text-muted italic flex items-center gap-2">
                ...
                Ïª¥Ìå©??Î™®Îìú?êÏÑú??Î©îÌ? ?∞Ïù¥?∞Í? Í∏∞Î≥∏Í∞íÏúºÎ°?Í∏∞Î°ù?©Îãà??
              </p>
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-between">
            <label className="inline-flex cursor-pointer items-center gap-3 select-none">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-admin-text-base uppercase tracking-widest">?êÎèô ?§Ìñâ Î°úÏßÅ</span>
                <span className="text-[9px] font-bold text-admin-text-muted italic">?êÎèô ?úÌÄÄ?§Î°ú ?§Ìñâ?òÎäî Î°úÍ∑∏?∏Ï? ?ïÏù∏?©Îãà??</span>
              </div>
            </label>
          </div>
            ...
            <div className="relative overflow-hidden rounded-xl border border-admin-danger/50 bg-admin-danger/5 p-5 animate-in fade-in zoom-in duration-300">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-2 flex-1">
                  <h4 className="text-xs font-bold text-admin-danger uppercase tracking-widest">2?®Í≥Ñ Î≥¥Ïïà ?ïÏù∏ (ÏµúÏ¢Ö ?ïÏù∏)</h4>
                  <p className="text-[11px] font-bold text-admin-text-muted leading-relaxed">
                    ÎØºÍ∞ê???¥ÏòÅ ?°ÏÖò?ºÎ°ú Í∞êÏ??òÏóà?µÎãà?? ?ÑÎûò ?§Ìñâ ?îÏïΩ???ïÏù∏????[Í∏∞Î°ù ?ïÏ†ï] Î≤ÑÌäº???åÎü¨Ï£ºÏÑ∏??
                  </p>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-admin-border">
          ...
          {isDangerous && !confirmArmed ? (
            ...
            >
              <Save size={18} />
              <span>{isDangerous ? "Í∏∞Î°ù ?ïÏ†ï (COMMIT)" : "Î°úÍ∑∏ Í∏∞Î°ù (DEPLOY)"}</span>
