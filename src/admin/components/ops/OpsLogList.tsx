...
import { Search, Filter, RefreshCw, Clock, Tag, FileJson } from "lucide-react";
...
const OpsLogList: React.FC = () => {
  ...
  const categoryColors: Record<string, string> = {
    [OpsLogCategory.GAME_PLAY]:
      "text-admin-brand bg-admin-brand/10 border-admin-brand/20",
    [OpsLogCategory.ECONOMY]:
      "text-admin-accent bg-admin-accent/10 border-admin-accent/20",
    [OpsLogCategory.SECURITY]:
      "text-admin-danger bg-admin-danger/10 border-admin-danger/20",
    [OpsLogCategory.SYSTEM]:
      "text-admin-text-secondary bg-admin-text-secondary/10 border-admin-text-secondary/20",
    [OpsLogCategory.USER_MANAGEMENT]:
      "text-admin-warning bg-admin-warning/10 border-admin-warning/20",
  };
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-admin-sidebar border border-admin-border rounded-lg flex-1 min-w-[200px]">
            ...
            <select
              ...
              onChange={(e) =>
                setSelectedCategory(e.target.value as OpsLogCategory)
              }
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 bg-admin-bg/90 backdrop-blur-sm z-10 border-b border-admin-border">
            <tr>
              <th className="p-4 text-xs font-bold text-admin-text-secondary uppercase">
                ?�간
              </th>
              <th className="p-4 text-xs font-bold text-admin-text-secondary uppercase">
                분류
            ...
            {isLoading ? (
              [...Array(5)].map((_, i) => (
                <tr key={i} className="animate-pulse">
                  <td className="p-4">
                    <div className="h-4 w-16 bg-admin-sidebar rounded"></div>
                  </td>
                  <td className="p-4">
                    <div className="h-6 w-24 bg-admin-sidebar rounded-full"></div>
                  </td>
                  <td className="p-4">
                    <div className="h-4 w-32 bg-admin-sidebar rounded"></div>
                  </td>
                  <td className="p-4">
                    <div className="h-4 w-20 bg-admin-sidebar rounded"></div>
                  </td>
                  <td className="p-4">
                    <div className="h-4 w-full bg-admin-sidebar rounded"></div>
                  </td>
                </tr>
              ))
            ) : !logs || logs.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="p-10 text-center text-admin-text-muted"
                >
                  ?�택??조건???�당?�는 로그가 ?�습?�다.
                </td>
              </tr>
              ...
              logs.map((log) => (
                <tr
                  key={log.id}
                  className="group hover:bg-admin-hover transition-colors"
                >
                  ...
                  <td className="p-4">
                    <span
                      className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${categoryColors[log.category] || "text-admin-text-muted bg-admin-sidebar border-admin-border"}`}
                    >
                      {log.category}
                    </span>
                  </td>
