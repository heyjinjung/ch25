...
const OpsDailyPlanPanel: React.FC = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      alert("?∞ÏùºÎ¶??åÎûú???Ä?•Îêò?àÏäµ?àÎã§.");
    },
    onError: (err) => {
      alert(`?Ä???§Ìå®: ${err} `);
    }
    ...
    <div className="admin-card-premium h-full flex flex-col">
      <div className="p-6 border-b border-admin-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-admin-subtitle text-admin-text-primary">?¥ÏòÅ ?∞ÏùºÎ¶??åÎûú</h2>
            <p className="text-xs text-admin-text-secondary">?ºÏùº ?åÎßà Î∞??∞ÏÑ†?úÏúÑ Í¥ÄÎ¶?/p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <label htmlFor="ops-daily-plan-date" className="sr-only">Ï°∞Ìöå ?†Ïßú</label>
          <input
        ...
        <div className="flex-1 p-6 space-y-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-2">
            <label className="text-admin-meta text-admin-text-secondary font-bold uppercase flex items-center gap-2">
              <Layout className="h-4 w-4" /> ?§Îäò???åÎßà
            </label>
            <input
              type="text"
              value={themeTitle}
              onChange={(e) => setThemeTitle(e.target.value)}
              placeholder="?? Ï£ºÎßê Î¶¨ÌÖê??Î∂Ä?§Ìä∏"
          ...
          <div className="space-y-2">
            <label className="text-admin-meta text-admin-text-secondary font-bold uppercase flex items-center gap-2">
              <CheckCircle2 className="h-4 w-4" /> ?ÅÌÉú
            </label>
            <div className="flex gap-2">
              {["PLANNING", "ACTIVE", "REVIEW", "CLOSED"].map((s) => (
                ...
                >
                  {s === "PLANNING" ? "Í≥ÑÌöç" : s === "ACTIVE" ? "ÏßÑÌñâ" : s === "REVIEW" ? "Î¶¨Î∑∞" : "Ï¢ÖÎ£å"}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2 flex-1 flex flex-col">
            <label className="text-admin-meta text-admin-text-secondary font-bold uppercase flex items-center gap-2">
              <AlignLeft className="h-4 w-4" /> ?åÎûú ?ÅÏÑ∏ / Î°úÍ∑∏
            </label>
            <textarea
              value={summaryMd}
              onChange={(e) => setSummaryMd(e.target.value)}
              placeholder="- ?∞ÏÑ†?úÏúÑ ?ëÏóÖ 1..."
