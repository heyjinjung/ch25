...
export const getV2ActiveSeason = async (): Promise<TeamBattleSeasonDto> => {
  try {
    const response = await userApi.get<TeamBattleSeasonDto>("/api/v2/team-battle/seasons/active");
...
export const getV2JoinableTeams = async (): Promise<TeamDto[]> => {
  try {
    const response = await userApi.get<TeamDto[]>("/api/v2/team-battle/teams");
    return response.data;
  } catch (error) {
...
export const joinV2Team = async (request: JoinTeamRequest): Promise<JoinTeamResponse> => {
  try {
    const response = await userApi.post<JoinTeamResponse>("/api/v2/team-battle/teams/join", request);
    return response.data;
  } catch (error) {
...
export const leaveV2Team = async (): Promise<LeaveTeamResponse> => {
  try {
    const response = await userApi.post<LeaveTeamResponse>("/api/v2/team-battle/teams/leave");
    return response.data;
  } catch (error) {
...
export const getV2MyTeamMembership = async (): Promise<TeamMembershipResponse> => {
  try {
    const response = await userApi.get<TeamMembershipResponse>("/api/v2/team-battle/teams/me");
    return response.data;
  } catch (error) {
...
export const getV2TeamLeaderboard = async (params?: LeaderboardParams): Promise<LeaderboardResponse> => {
  try {
    const response = await userApi.get<LeaderboardResponse>("/api/v2/team-battle/teams/leaderboard", {
      params: {
