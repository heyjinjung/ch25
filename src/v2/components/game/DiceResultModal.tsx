...
export default function DiceResultModal({
  ...
  return (
    ...
    >
      ...
      >
        {/* Glow Effects */}
        <div className={`absolute top-0 inset-x-0 h-[100px] bg-gradient-to-b ${isWin ? "from-emerald-500/20" : isDraw ? "from-amber-500/20" : "from-red-500/20"} to-transparent pointer-events-none`} />
        
        {/* Content */}
        {renderIcon()}
        
        <h2 className={`text-3xl font-black ${titleColor} italic tracking-tighter mb-2`}>
          {titleText}
        </h2>
        ...
        {vaultEarn > 0 && (
            <div className="w-full bg-white/5 rounded-xl p-3 mb-6 border border-white/5 flex flex-col items-center">
                <span className="text-xs text-zinc-500 font-bold uppercase tracking-wider mb-1">Earned Reward</span>
                <div className="text-2xl font-black text-white flex items-center gap-1">
                    +{vaultEarn.toLocaleString()} <span className="text-sm font-bold text-zinc-400">PTS</span>
                </div>
            </div>
        )}
