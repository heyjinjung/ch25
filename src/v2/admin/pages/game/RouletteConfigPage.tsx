...
export default function RouletteConfigPage() {
  ...
  const handleSave = () => {
    ...
    if (totalWeight <= 0) {
      alert("Ï¥?Í∞ÄÏ§ëÏπò??0Î≥¥Îã§ Ïª§Ïïº ?©Îãà??");
      return;
    }
  ...
  const grades: { value: RouletteGrade; label: string; color: string }[] = [
    { value: "COMMON", label: "?ºÎ∞ò (Common)", color: "text-zinc-400" },
    { value: "VIP", label: "VIP", color: "text-yellow-400" },
    { value: "WHALE", label: "WHALE (Í≥†Îûò)", color: "text-purple-400" },
    { value: "AT_RISK", label: "Í¥ÄÎ¶??Ä??(At Risk)", color: "text-red-400" },
  ];
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-bold tracking-tight mb-1 flex items-center gap-2">
            ...
            Î£∞Î†õ ?§Ï†ï (Roulette Config SoT)
          </h1>
          <p className="text-sm text-zinc-400">
            ?±Í∏âÎ≥?Î£∞Î†õ ?ïÏ±Ö Î∞??∏Í∑∏Î®ºÌä∏ Í∞ÄÏ§ëÏπòÎ•?Í¥ÄÎ¶¨Ìï©?àÎã§. (V2 Specs: Grade &
            Weight)
          </p>
        </div>
        <Button
        ...
        >
          ...
          {updateMutation.isPending ? "?Ä??Ï§?.." : "?§Ï†ï ?Ä??}
        </Button>
      </div>
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/5">
              <div className="space-y-0.5">
                <Label className="text-base">Í≤åÏûÑ ?úÏÑ±??/Label>
                <p className="text-xs text-zinc-500">
                  ÎπÑÌôú?±Ìôî ???êÍ? Ï§?Î©îÏãúÏßÄÍ∞Ä ?úÏãú?©Îãà??
                </p>
              </div>
              <Switch
            ...
            <div className="space-y-2">
              <Label>?ÖÏû• ?¨Ìôî (Ticket Type)</Label>
              <Select
            ...
            <div className="space-y-2">
              <Label>?ºÏùº ÏµúÎ? ?åÏ†Ñ??(Daily Max)</Label>
            ...
            <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-lg">
              <div className="text-xs text-blue-300 space-y-1">
                <p className="font-semibold mb-2 flex items-center gap-1">
                  <Info className="w-3 h-3" /> ?±Í∏âÎ≥?Í∞Ä?¥Îìú
                </p>
                <p>??COMMON: ?ºÎ∞ò Î£∞Î†õ (1~3??</p>
                <p>??VIP/WHALE: Í≥®Îìú/?§Ïù¥??Î£∞Î†õ (Í≥†Í?Ïπ?Î≥¥ÏÉÅ)</p>
                <p>??AT_RISK: ?ÑÍ∏∞ Í∞úÏûÖ??(?πÏ≤® ?ïÎ•† ?ÅÌñ• Ï°∞Ï†ï Í∂åÏû•)</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div className="space-y-1">
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                ?¨Î°Ø Í∞ÄÏ§ëÏπò ?§Ï†ï (6 Slots Fixed)
              </CardTitle>
              <CardDescription>
                Í∞ÄÏ§ëÏπò(Weight)Î•??ÖÎ†•?òÎ©¥ ?ïÎ•†(%)?Ä ?êÎèô Í≥ÑÏÇ∞?©Îãà??
              </CardDescription>
            </div>
            ...
            {totalWeight === 0 && (
              <div className="mt-4 flex items-center justify-center p-3 bg-red-500/10 border border-red-500/20 rounded text-red-400 gap-2">
                ...
                <span className="font-semibold">
                  Í≤ΩÍ≥†: Í∞ÄÏ§ëÏπò ?©Ïù¥ 0?ÖÎãà?? Í≤åÏûÑ???ïÏÉÅ ?ôÏûë?òÏ? ?äÏäµ?àÎã§.
                </span>
              </div>
            )}
          </CardContent>
