...
import { useRouletteConfigs, useUpdateRouletteConfig } from "../../../hooks/useAdminGameConfig";
import { type AdminRouletteConfigDto, type AdminRouletteSegmentDto, type RouletteGrade } from "../../../api/adminApi";
...
export default function RouletteConfigPage() {
  ...
  const [activeConfig, setActiveConfig] = useState<AdminRouletteConfigDto | null>(null);
  ...
  const handleConfigChange = (field: keyof AdminRouletteConfigDto, value: any) => {
    if (!activeConfig) return;
    setActiveConfig({ ...activeConfig, [field]: value });
  };

  const handleSegmentChange = (slotIndex: number, field: keyof AdminRouletteSegmentDto, value: any) => {
    if (!activeConfig) return;
  ...
  const handleSave = () => {
    if (!activeConfig) return;
    const totalWeight = activeConfig.segments.reduce((sum, s) => sum + s.weight, 0);
    if (totalWeight <= 0) {
  ...
  if (isLoading) return <div className="text-center py-20 text-zinc-500 animate-pulse">Loading configurations...</div>;
  if (!activeConfig) return <div className="text-center py-20 text-zinc-500">No configuration found for {selectedGrade}</div>;

  const totalWeight = activeConfig.segments.reduce((sum, s) => sum + s.weight, 0) || 1;
    ...
    <div className="space-y-6 text-white p-6 h-full overflow-y-auto">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-zinc-400">등급별 룰렛 정책 및 세그먼트 가중치를 관리합니다. (V2 Specs: Grade & Weight)</p>
        </div>
        <Button 
          onClick={handleSave} 
        ...
        {grades.map((grade) => (
            <button
                ...
                className={cn(
                    ...
                    selectedGrade === grade.value && grade.value !== "COMMON" && grade.color
                )}
            >
                {grade.label}
            </button>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/5">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-zinc-500">비활성화 시 점검 중 메시지가 표시됩니다.</p>
                </div>
                <Switch 
              ...
              <div className="space-y-2">
                ...
                <Select value={activeConfig.ticketType} onValueChange={(val) => handleConfigChange("ticketType", val)}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input 
                    ...
                    onChange={(e) => handleConfigChange("maxDailySpins", parseInt(e.target.value))}
                    className="bg-black/50 border-white/10 text-right h-10 font-mono" 
                  />
                  <span className="text-sm text-zinc-500 whitespace-nowrap">회 / 일</span>
                </div>
              </div>
          ...
          <Card className="lg:col-span-2 bg-[#18181B] border-white/5 shadow-xl">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="space-y-1">
                ...
                <CardDescription>가중치(Weight)를 입력하면 확률(%)은 자동 계산됩니다.</CardDescription>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold font-mono text-emerald-400">{totalWeight}</div>
                <div className="text-xs text-zinc-500 uppercase tracking-wider">Total Weight</div>
              </div>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {activeConfig.segments.sort((a,b) => a.slotIndex - b.slotIndex).map((segment) => {
                    const probability = ((segment.weight / totalWeight) * 100).toFixed(1);
                    return (
                      <TableRow key={segment.slotIndex} className="border-white/5 hover:bg-white/5 group">
                        <TableCell className="font-mono text-zinc-500 text-center bg-black/20">{segment.slotIndex}</TableCell>
                        <TableCell>
                          <Input
                            value={segment.label}
                            onChange={(e) => handleSegmentChange(segment.slotIndex, "label", e.target.value)}
                        ...
                        <TableCell>
                           <Badge variant="outline" className="bg-zinc-800/50 border-zinc-700 text-zinc-300 group-hover:bg-zinc-800 transition-colors">
                             {segment.rewardType}
                           </Badge>
                        </TableCell>
                        <TableCell>
                          <Input
                            ...
                            onChange={(e) => handleSegmentChange(segment.slotIndex, "rewardAmount", parseFloat(e.target.value))}
                        ...
                        <TableCell>
                          <div className="relative">
                            <Input
                              ...
                              onChange={(e) => handleSegmentChange(segment.slotIndex, "weight", parseInt(e.target.value))}
                              className="h-8 bg-black/20 border-white/5 w-[80px] font-bold text-emerald-400 text-center focus:border-emerald-500"
                            />
                          </div>
                        </TableCell>
                        <TableCell>
                          <span className="text-sm font-mono text-zinc-300">{probability}%</span>
                        </TableCell>
                        <TableCell className="text-center">
                          <Switch
                            checked={segment.isJackpot}
                            onCheckedChange={(checked) => handleSegmentChange(segment.slotIndex, "isJackpot", checked)}
              ...
              {totalWeight === 0 && (
                <div className="mt-4 flex items-center justify-center p-3 bg-red-500/10 border border-red-500/20 rounded text-red-400 gap-2">
                  ...
                  <span className="font-semibold">경고: 가중치 합이 0입니다. 게임이 정상 동작하지 않습니다.</span>
                </div>
              )}
            </CardContent>
          </Card>
