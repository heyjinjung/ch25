...
import { type AdminLevelDto, getWalletTransactionTypes } from "../../../api/adminApi";
    ...
    <section className="bg-[#09090b] min-h-screen pb-20 p-6 md:p-8 max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8">
        <div className="space-y-2">
          ...
          <p className="text-zinc-400 text-base max-w-2xl">
            사용자 레벨별 경험치 요구량 및 달성 보상을 설정합니다.<br/>
            <span className="text-zinc-500 text-sm">보상 종류와 수량을 유동적으로 변경할 수 있습니다.</span>
          </p>
        </div>

        <div className="flex items-center gap-4">
          {/* Global Stats / Config Trigger */}
          <div className="flex items-center gap-6 bg-[#18181b] px-6 py-3 rounded-2xl border border-white/10 shadow-sm hover:border-indigo-500/30 transition-colors cursor-pointer group"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
        {filteredLevels.map((level) => (
          ...
          >
            ...
            <div className="absolute top-0 left-0 h-1.5 bg-zinc-800 w-full z-10">
              <div
                className="h-full bg-indigo-500 transition-all opacity-75 group-hover:opacity-100"
                style={{
                  width: `${(level.level / levelSummary.maxLevel) * 100}%`,
                }}
              />
            </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            <input
                                ...
                                onBlur={(e) =>
                                    handleUpdate(level.level, "requiredXp", parseInt(e.target.value))
                                }
                        ...
                        <div className="grid grid-cols-[1.5fr_1fr] gap-2">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    onChange={(e) => handleUpdate(level.level, "rewardType", e.target.value)}
                                >
                                    {rewardTypes?.map((t) => (
                                        <option key={t.value} value={t.value}>{t.label}</option>
                                    ))}
                                    {/* Fallback Legacy Types if not fetched yet */}
                                    {!rewardTypes && (
                                        <>
                                            <option value="POINT">포인트 (Point)</option>
                                            <option value="ROULETTE_COIN">룰렛 티켓</option>
                                        </>
                                    )}
                                </select>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    onBlur={(e) => handleUpdate(level.level, "rewardAmount", parseInt(e.target.value))}
        ...
        {filteredLevels.length === 0 && (
          <div className="col-span-full h-80 flex flex-col items-center justify-center gap-6 text-zinc-500 bg-[#18181b] rounded-3xl border border-white/5 border-dashed">
            ...
            <div className="text-center">
                <p className="text-lg font-bold text-zinc-400">검색 결과가 없습니다.</p>
      ...
      <Dialog open={isGlobalConfigOpen} onOpenChange={setIsGlobalConfigOpen}>
        <DialogContent className="bg-[#18181B] border-white/10 text-white sm:max-w-[425px]">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setIsGlobalConfigOpen(false)} className="text-zinc-400 hover:text-white">
                취소
            </Button>
            <Button onClick={handleSaveGlobalConfig} className="bg-indigo-600 hover:bg-indigo-700 text-white" disabled={globalConfigMutation.isPending}>
