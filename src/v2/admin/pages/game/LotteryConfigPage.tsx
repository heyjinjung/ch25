...
import { Ticket, Save, Info, Settings, Grid2x2, Trophy, Plus, Trash2, Zap } from "lucide-react";
...
export default function LotteryConfigPage() {
  ...
  const handlePrizeChangeLocal = (id: number, field: keyof AdminLotteryPrizeDto, value: any) => {
  ...
  const handleUpdatePrize = (prizeId: number) => {
    ...
    if (!prize) return;
    updatePrizeMutation.mutate({ configId: localConfig.id, prizeId, data: prize });
  };
  ...
  if (isLoading) return <div className="text-center py-20 text-zinc-500 animate-pulse font-mono tracking-widest uppercase">Initializing Strategy...</div>;
    ...
    <div className="admin-page-container custom-scrollbar">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-2 ml-1">즉석 복권의 당첨 확률, 재고 및 이벤트를 실시간으로 관리합니다.</p>
        </div>
        <Button 
          ...
          <CardContent className="space-y-6 flex-1">
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5 group hover:border-white/10 transition-colors">
              <div className="space-y-0.5">
                <Label className="text-base text-zinc-200 font-bold">복권 서비스 활성화</Label>
                <p className="text-xs text-zinc-500 italic">비활성 시 상점에서 상품이 숨겨집니다.</p>
              </div>
              <Switch 
            ...
            <div className="space-y-3">
               ...
               <div className="relative">
                 ...
                 />
                 <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-zinc-500 font-bold">TICKET</span>
               </div>
            </div>
            ...
            <div className="pt-4 border-t border-white/5">
               <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-bold text-purple-400 uppercase tracking-widest">Puzzle Event</span>
               </div>
               <div className="p-5 rounded-2xl bg-purple-500/5 border border-purple-500/10 space-y-4">
                  <div className="flex justify-between items-end">
                    <Label className="text-zinc-400 text-[10px] font-black uppercase">Drop Probability</Label>
            ...
            <div className="pt-4 border-t border-white/5 flex flex-col items-center justify-center py-6 bg-gradient-to-b from-transparent to-zinc-900/50 rounded-b-xl">
               <div className="flex items-center gap-2 mb-1">
                 ...
                 <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Active Total Weight</span>
               </div>
               <div className="text-5xl font-black font-mono text-zinc-100 tracking-tighter shadow-blue-500/50 drop-shadow-xl">
                 {totalWeight}
               </div>
               <p className="text-[10px] text-zinc-600 mt-2">비활성화된 보상은 계산에서 제외됩니다.</p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
              ...
              <Table>
                <TableHeader className="bg-zinc-900/80">
                  <TableRow className="border-white/5 hover:bg-transparent">
                    <TableHead className="w-[60px] text-center text-zinc-500 font-bold text-[10px] uppercase">Status</TableHead>
                    <TableHead className="text-zinc-500 font-bold text-[10px] uppercase">Prize Label</TableHead>
                    <TableHead className="text-zinc-500 font-bold text-[10px] uppercase">Reward Type</TableHead>
                    <TableHead className="text-zinc-500 font-bold text-[10px] uppercase text-right">Value</TableHead>
                    <TableHead className="w-[80px] text-center text-zinc-500 font-bold text-[10px] uppercase">Stock</TableHead>
                    <TableHead className="w-[100px] text-center text-zinc-500 font-bold text-[10px] uppercase">Weight</TableHead>
                    <TableHead className="w-[80px] text-right text-zinc-500 font-bold text-[10px] uppercase">Prob%</TableHead>
                    <TableHead className="w-[80px] text-center text-zinc-500 font-bold text-[10px] uppercase">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {localConfig.prizes.map((prize) => {
                    const probability = prize.isActive 
                       ...
                       : "0.00";
                    return (
                      <TableRow key={prize.id} className="border-white/5 hover:bg-white/5 group transition-colors">
                        <TableCell className="text-center">
                          <Switch 
                            checked={prize.isActive} 
                            onCheckedChange={(v) => handlePrizeChangeLocal(prize.id, "isActive", v)}
                        ...
                        <TableCell>
                          <Input
                            value={prize.label}
                            onChange={(e) => handlePrizeChangeLocal(prize.id, "label", e.target.value)}
                        ...
                        <TableCell>
                           <Select
                             ...
                             onValueChange={(v) => handlePrizeChangeLocal(prize.id, "rewardType", v)}
                           >
                             ...
                             <SelectContent className="bg-[#18181b] border-white/10 text-zinc-200">
                               {REWARD_ITEMS.map((item) => (
                                 <SelectItem key={item.value} value={item.value} className="text-xs hover:bg-white/5">
                                   {item.label}
                                 </SelectItem>
                               ))}
                             </SelectContent>
                        ...
                        <TableCell>
                          <Input
                            ...
                            onChange={(e) => handlePrizeChangeLocal(prize.id, "rewardAmount", parseFloat(e.target.value))}
                        ...
                        <TableCell>
                          <div className="relative flex justify-center">
                            ...
                            {prize.stock === undefined && (
                               <span className="absolute inset-0 flex items-center justify-center pointer-events-none text-zinc-600 font-black">∞</span>
                            )}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Input
                            type="number"
                            value={prize.weight}
                            onChange={(e) => handlePrizeChangeLocal(prize.id, "weight", parseInt(e.target.value))}
                            className="h-9 bg-zinc-900 border-white/5 w-full font-black text-amber-500 text-center font-mono"
                          />
                        </TableCell>
                        <TableCell className="text-right">
                          <span className={`text-xs font-mono font-bold ${prize.isActive ? 'text-emerald-400' : 'text-zinc-600'}`}>
                             {probability}%
                          </span>
                        </TableCell>
                        <TableCell>
            ...
            <div className="mt-6 p-4 bg-zinc-900/50 border border-white/5 rounded-2xl flex items-center gap-4">
               ...
               <div className="flex-1">
                 <h4 className="text-xs font-bold text-zinc-200">데이터 동기화 알림</h4>
                 <p className="text-[10px] text-zinc-500 mt-0.5 leading-relaxed">
                   각 보상 행의 저장 버튼을 누르면 즉시 DB에 반영됩니다. 상단 '전역 설정 저장'은 이름, 활성 상태, 일일 한도 전용입니다.
                 </p>
               </div>
            </div>
          </CardContent>
