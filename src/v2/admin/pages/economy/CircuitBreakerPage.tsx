    ...
    <div className="space-y-8 p-4 md:p-8 bg-[#121214] min-h-screen text-zinc-200">
          ...
          return (
            <Card key={status.assetType} className={`bg-[#18181B] border-white/5 transition-all overflow-hidden ${isCritical ? 'ring-2 ring-red-500/50' : ''}`}>
              <CardHeader className="pb-4 border-b border-white/5">
                ...
                <CardDescription className="text-zinc-500 text-xs">글로벌 지급 임계치 (Threshold)</CardDescription>
              </CardHeader>
              <CardContent className="pt-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 py-2 border-y border-white/[0.03]">
                  <div>
                    <span className="block text-[10px] text-zinc-500 uppercase">유저 한도 설정 (User Limit)</span>
                    <span className="text-sm font-semibold text-zinc-300">{status.config.userLimit.toLocaleString()}</span>
                  </div>
                  <div className="text-right">
                    <span className="block text-[10px] text-zinc-500 uppercase">글로벌 한도 설정 (Global Limit)</span>
      ...
      <div className="p-4 bg-zinc-950/50 rounded-lg border border-white/5 flex items-center gap-4">
        ...
        <div className="text-sm">
          <span className="text-zinc-300 font-medium font-bold">운영 팁 (Operation Tip):</span>
