...
export default function CCDepositPage() {
  ...
  const resolveRowUser = async (row: EditableRow) => {
    ...
    try {
      ...
      if (response.users && response.users.length > 0) {
        ...
        setResolveStatusByKey((prev) => ({
          ...prev,
          [row.__key]: { state: "error", message: "?†Ï?Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§." },
        }));
      }
    } catch {
      setResolveStatusByKey((prev) => ({
        ...prev,
        [row.__key]: { state: "error", message: "?†Ï? Í≤Ä???§Î•ò" },
      }));
    }
  };
  ...
  const handleSaveAll = async () => {
    ...
    if (invalidRows.length > 0) {
      alert("?†Ï? Í≤ÄÏ¶?Í∏àÏï°/?†ÏßúÍ∞Ä ÎπÑÏñ¥ ?àÎäî ?âÏù¥ ?àÏäµ?àÎã§.");
      return;
    }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-zinc-400">
            ?òÎèô?ºÎ°ú ?ÖÍ∏à ?¥Ïó≠???ùÏÑ±?òÍ±∞?? ?òÎ™ª Í∏∞ÏûÖ???¥Ïó≠???òÏ†ï/??†ú?©Îãà??
          </p>
        </div>
            ...
            <TableRow className="border-white/5 hover:bg-transparent">
              ...
              >
                <div className="flex items-center">
                  ?†Ï? {getSortIcon("nickname")}
                </div>
              </TableHead>
              <TableHead className="text-zinc-400">?ÑÏ†Å</TableHead>
              <TableHead
                className="text-zinc-400 cursor-pointer hover:text-white transition-colors"
                onClick={() => handleSort("amount")}
              >
                <div className="flex items-center">
                  Í∏àÏï° (KRW) {getSortIcon("amount")}
                </div>
              </TableHead>
              <TableHead
                className="text-zinc-400 cursor-pointer hover:text-white transition-colors"
              ...
              >
                <div className="flex items-center">
                  ?ÖÍ∏à ?†Ïßú (KST) {getSortIcon("kstDate")}
                </div>
              </TableHead>
              <TableHead
                className="text-zinc-400 cursor-pointer hover:text-white transition-colors"
              ...
              >
                <div className="flex items-center">
                  ?ëÏóÖ?ºÏãú {getSortIcon("createdAt")}
                </div>
              </TableHead>
              <TableHead className="text-zinc-400">Í≤ÄÏ¶?/TableHead>
          ...
          <TableBody>
            {sortedRows.length === 0 ? (
              <TableRow>
                ...
                >
                  {isLoading ? "Î∂àÎü¨?§Îäî Ï§?.." : "?ÖÍ∏à ?¥Ïó≠???ÜÏäµ?àÎã§."}
                </TableCell>
              </TableRow>
            ) : (
              sortedRows.map((row) => {
                  ...
                  >
                    <TableCell className="py-[5px]">
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="text-xs text-zinc-500">
                          UID: {row.userId ?? "-"} / {row.nickname ?? "ÎØ∏Ìôï??}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="py-[5px]">
                    ...
                    <TableCell className="text-xs py-[5px]">
                      {status?.state === "loading" && (
                        <span className="text-zinc-500">Í≤ÄÏ¶?Ï§?..</span>
                      )}
                      {status?.state === "ok" && (
                        <span className="text-emerald-400">?ïÏù∏??/span>
                      )}
                      {status?.state === "error" && (
                        <span className="text-red-400">{status.message}</span>
                      )}
                      {status?.state === "idle" && (
                        <span className="text-zinc-500">?ÄÍ∏?/span>
                      )}
                    </TableCell>
