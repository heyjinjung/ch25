          ...
          <>
                  ...
                  <div>
                    <SheetTitle className="text-white text-lg font-bold flex items-center gap-2">
                      ...
                      {user.vipLevel === "VIP" ? (
                        ...
                        <Badge
                          variant="secondary"
                          className="bg-zinc-500/10 text-zinc-500 border-zinc-500/20 text-[10px] h-5"
                        >
                          COMMON
                        </Badge>
                      )}
            ...
            <Tabs defaultValue="overview" className="h-[calc(100vh-80px)]">
              <TabsList className="w-full justify-start rounded-none bg-[#18181B] border-b border-white/5 px-6 h-12 gap-6">
                <TabsTrigger
                  value="overview"
                  className="rounded-none border-b-2 border-transparent data-[state=active]:border-indigo-500 data-[state=active]:bg-transparent data-[state=active]:text-indigo-400 px-0 pb-3 pt-2 text-zinc-400"
                >
                  개요 (Overview)
                </TabsTrigger>
                <TabsTrigger
                ...
                <TabsTrigger
                  value="intervention"
                  className="rounded-none border-b-2 border-transparent data-[state=active]:border-red-500 data-[state=active]:bg-transparent data-[state=active]:text-red-400 px-0 pb-3 pt-2 text-zinc-400"
                >
                  개입 (Intervention)
                </TabsTrigger>
                <TabsTrigger
                    ...
                    <div className="p-6 rounded-xl border border-red-500/10 bg-red-500/5 space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-bold text-white">
                          운영 개입 플레이북 (Operational Playbook)
                        </h3>
                      </div>
                      <p className="text-sm text-zinc-400">
                        시스템이 유저의 최근 행동 패턴을 분석하여 제안하는 대응
                        방안입니다. 우선순위에 따라 신중하게 실행하십시오.
                      </p>
                    </div>
                    ...
                    {user.playbook ? (
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider px-1">
                          추천 액션 (Suggested Actions)
                        </h4>
                        <div className="grid gap-3">
                          {user.playbook.suggestedActions.map((action) => (
                            <Card
                              key={action.actionId}
                              className="bg-[#18181B] border-white/5 hover:border-indigo-500/30 transition-colors"
                            >
            ...
            onUpdate={async (amt, reason) => {
              if (user) {
                await adjustWallet.mutateAsync({
                  userId: user.id,
                  request: {
                    amount: amt - user.ticketBalance, // amt is new total, we need the delta
