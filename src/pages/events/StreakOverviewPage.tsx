...
const StreakOverviewPage: React.FC = () => {
    ...
    const handleClaim = async () => {
        try {
            ...
            if (res.success) {
                addToast("ë³´ìƒ???ë“?ˆìŠµ?ˆë‹¤!", "success");
                queryClient.invalidateQueries({ queryKey: ['streak-rules'] });
                fetchMissions(); // Refresh store
                return true;
            }
        } catch {
            addToast("ë³´ìƒ ?ë“ ?¤íŒ¨", "error");
        }
        return false;
    };

    if (isLoading) return <div className="flex justify-center items-center h-screen bg-black"><Loader2 className="animate-spin text-white w-8 h-8" /></div>;
    ...
    return (
        <div className="min-h-screen bg-black pb-20">
            ...
            <div className="sticky top-0 z-10 bg-black/80 backdrop-blur-md border-b border-white/5 px-4 h-14 flex items-center gap-3">
                ...
                <h1 className="text-lg font-bold text-white">?¤íŠ¸ë¦?ëª¨ì•„ë³´ê¸°</h1>
            </div>
