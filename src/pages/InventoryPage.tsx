...
const InventoryPage: React.FC = () => {
    ...
    const useMutationAction = useMutation({
        ...
        onSuccess: (data) => {
            tryHaptic(20);
            addToast(`?¨Ïö© ?ÑÎ£å: ${data.reward_token} x${data.reward_amount} ÏßÄÍ∏âÎê®`, "success");
            queryClient.invalidateQueries({ queryKey: ['inventory'] });
            queryClient.invalidateQueries({ queryKey: ['vault-status'] });
        },
        onError: (error: any) => {
            const msg = error.response?.data?.detail || "?¨Ïö© ?§Ìå®";
    ...
    const useCraftAction = useMutation({
        ...
        onSuccess: (data: any) => {
            tryHaptic(20);
            addToast(`?úÏûë ?ÑÎ£å: ${data.reward_token} x${data.reward_amount}`, "success");
            queryClient.invalidateQueries({ queryKey: ['inventory'] });
        },
        onError: (error: any) => {
            const msg = error.response?.data?.detail || "?úÏûë ?§Ìå®";
            tryHaptic(50);
            addToast(msg, "error");
        }
    });
    ...
    if (isLoading) {
        return (
            <div className="mx-auto w-full max-w-lg py-16 flex flex-col items-center justify-center">
                ...
                <p className="mt-4 text-white/50 text-sm font-medium">?∏Î≤§?†Î¶¨ Î∂àÎü¨?§Îäî Ï§?..</p>
                {/* Joyride ?ÄÍ≤??ÑÎ¶¨?Ä??(Î°úÎî© Ï§ëÏóê???¨Îûò??Î∞©Ï?) */}
        ...
        return (
            <div className="mx-auto w-full max-w-lg pb-[calc(96px+env(safe-area-inset-bottom))]">
                <div className="rounded-[24px] border border-white/10 bg-white/5 p-5 text-center">
                    ...
                    <div className="text-sm font-black text-white/90">?∞Ïù¥??Î°úÎî© ?§Ìå®</div>
                    <div className="mt-1 text-[11px] font-medium text-white/50">?∏Î≤§?†Î¶¨ ?ïÎ≥¥Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??</div>
                </div>
            </div>
        );
    }
        ...
        <div className="mx-auto w-full max-w-lg min-h-screen bg-black pb-[calc(96px+env(safe-area-inset-bottom))]">
            ...
            <div className="sticky top-0 z-40 bg-black/80 backdrop-blur-xl border-b border-white/5 px-4 h-14 flex items-center justify-between" data-tour="inventory-link">
                <h1 className="text-[15px] font-bold text-white flex items-center gap-1.5 tracking-tight">
                    <img src="/assets/icon_inventory_wallet.png" className="w-5 h-5 object-contain" alt="Î≥¥ÏÉÅ?? />
                    Î≥¥ÏÉÅ??
                </h1>
                <button
                ...
                >
                    <img src="/assets/icons/icon_cart.png" className="w-5 h-5 object-contain opacity-70" alt="ÍµêÌôò?? />
                    ÍµêÌôò??
                </button>
            </div>
            ...
            <div className="mb-6" data-tour="inventory-tabs">
                <div className="relative flex p-1.5 bg-gradient-to-r from-white/5 to-white/[0.03] rounded-2xl border border-white/10 shadow-inner">
                    ...
                    >
                        <span className="relative z-10">Î≥¥Ïú†??/span>
            ...
            <div className="space-y-6 animate-fadeIn">
                <div>
                    <div className="mb-3 flex items-baseline justify-between">
                        <h2 className="text-sm font-black text-white/90">Î≥¥Ïú† ?ÑÏù¥??/h2>
                    </div>
                    <div className="flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-hide -mx-1 px-1">
                        {items.filter(it => it.quantity > 0).length === 0 ? (
                            <div className="w-full flex flex-col items-center justify-center py-12 space-y-3">
                                ...
                                <p className="text-white/20 text-sm font-bold tracking-tight">Î≥¥Ïú†???ÑÏù¥?úÏù¥ ?ÜÏäµ?àÎã§</p>
                            </div>
                        ) : (
                            items
                ...
                <div>
                    <div className="mb-3 flex items-baseline justify-between">
                        <h2 className="text-[14px] font-black text-white/90">?∞Ïºì ÏßÄÍ∞?/h2>
                    </div>
                    <div className="flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-hide -mx-1 px-1">
                        {[
                            'ROULETTE_COIN',
...
const ItemCard: React.FC<ItemCardProps> = ({ item, onUse, isPending }) => {
    const GIFTICON_BRAND_LABEL: Record<string, string> = {
        ...
        CU: "CU",
        GS25: "GS25",
        COMPOSE_AMERICANO: "Ïª¥Ìè¨Ï¶??ÑÏïÑ",
    };
    ...
    const getGifticonInfo = (itemType: string) => {
        ...
        const brandLabel = GIFTICON_BRAND_LABEL[brandCodeRaw] ?? "Í∏∞ÌîÑ?∞ÏΩò";
        const title = amount
            ? `${brandLabel}`
            : `${brandLabel}`;

        const sub = amount ? `${amount.toLocaleString()}?? : "Í∏∞ÌîÑ?∞ÏΩò";
                ...
                <div className="flex flex-col relative z-10 h-full items-center text-center p-4">
                    <div className="relative mb-4 mt-2" style={{ transform: "translateZ(20px)" }}>
                        ...
                        <div className="absolute -top-1.5 -right-1.5 bg-white text-black text-[10px] font-black px-1.5 py-0.5 rounded-full ring-2 ring-black tabular-nums shadow-lg">
                            √ó{item.quantity.toLocaleString()}
                        </div>
                    </div>
                    ...
                    >
                        {item.item_type === "DIAMOND" ? "Î≥¥Ïú†Ï§? : (item.item_type.includes("FRAGMENT") ? "?úÏûë?òÍ∏∞ (Craft)" : (isPendingFulfillment ? "ÏßÄÍ∏âÎ?Í∏? : (isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : "?¨Ïö©?òÍ∏∞")))}
                    </button>
                </div>
            </div>
        </motion.div>
...
const WalletCard: React.FC<{ tokenType: string; amount: number }> = ({ tokenType, amount }) => {
    const WALLET_INFO: Record<string, { title: string; icon: string }> = {
        "ROULETTE_COIN": { title: "Î£∞Î†õ ?∞Ïºì", icon: "/assets/asset_ticket_green.png" },
        "DICE_TOKEN": { title: "Ï£ºÏÇ¨???∞Ïºì", icon: "/assets/icon_dice_silver.png" },
        "LOTTERY_TICKET": { title: "Î≥µÍ∂å ?∞Ïºì", icon: "/assets/lottery/icon_lotto_ball.webp" },
        "GOLD_KEY": { title: "Í≥®Îìú ??, icon: "/assets/icons/goldkey.png" },
        "DIAMOND_KEY": { title: "?§Ïù¥????, icon: "/assets/icons/diakey.png" },
        "TRIAL_TOKEN": { title: "Ï≤¥Ìóò ?∞Ïºì", icon: "/assets/asset_ticket_trial.png" }
    };
        ...
        <div className="relative group overflow-hidden bg-gradient-to-b from-white/[0.08] to-white/[0.02] border border-white/10 rounded-[22px] p-4 transition-all active:scale-[0.98] hover:border-white/20">
            <div className="flex flex-col relative z-10 items-center text-center">
                ...
                <div className="flex items-baseline justify-center gap-1">
                    ...
                    <span className="text-[10px] font-bold text-white/30">Í∞?/span>
                </div>
            </div>
