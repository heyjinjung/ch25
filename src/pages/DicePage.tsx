...
const DicePage: React.FC = () => {
  ...
  const mapErrorMessage = (err: any) => {
    ...
    if (code === "NO_FEATURE_TODAY") return "?¤ëŠ˜ ?¤ì •???´ë²¤?¸ê? ?†ìŠµ?ˆë‹¤.";
    if (code === "INVALID_FEATURE_SCHEDULE")
      return "?´ë²¤???¤ì?ì¤„ì´ ?˜ëª»?˜ì—ˆ?µë‹ˆ?? ì§€ë¯¼ì´?ê²Œ ë¬¸ì˜?˜ì„¸??";
    if (code === "FEATURE_DISABLED") return "?´ë²¤?¸ê? ë¹„í™œ?±í™”?˜ì—ˆ?µë‹ˆ??";
    if (code === "DAILY_LIMIT_REACHED")
      return "?¤ëŠ˜ ì°¸ì—¬ ?Ÿìˆ˜ë¥?ëª¨ë‘ ?¬ìš©?ˆìŠµ?ˆë‹¤.";
    if (code === "NOT_ENOUGH_TOKENS")
      return "?°ì¼“??ë¶€ì¡±í•©?ˆë‹¤. ì§€ë¯¼ì´?ê²Œ ì¶©ì „?´ë‹¬?¼ê³  ?¼ì¨ë³´ì„¸??";
    return "ë°°í? ì¤€ë¹?ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤. ?¤ì‹œ ?œë„?´ì£¼?¸ìš”.";
  };
  ...
  const handlePlay = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        // [UX FIX] ì¤‘ë³µ ëª¨ë‹¬ ë°©ì?: ë³´ìƒ ?ë“ ??ë³´ìƒ ? ìŠ¤?¸ê? ?¨ë?ë¡?ê¸ˆê³  ?ë¦½ ëª¨ë‹¬?€ ìµœì†Œ??
        if (response.eventSeeded && response.eventSeedAmount) {
          setVaultModal({
            open: true,
            amount: response.eventSeedAmount,
            title: "?´ë²¤??ì²?ì°¸ì—¬ ?œë“œ ë³´ë„ˆ??,
          });
    ...
    if (isError || !data) {
      return (
        <div className="rounded-[2.5rem] border border-white/5 bg-white/5 p-12 text-center backdrop-blur-3xl">
          ...
          <p className="text-2xl font-black text-white">?°ì´???™ê¸°???¤íŒ¨</p>
          <p className="mt-2 text-white/40">
            ?µì‹  ?íƒœë¥??•ì¸?˜ê³  ?¤ì‹œ ?…ì¥?´ì£¼?¸ìš”.
          </p>
        </div>
      );
    }
  ...
  return (
    <FeatureGate feature="DICE">
      <GamePageShell title="ì£¼ì‚¬??ë°°í?" px="p-3 sm:p-5">
          ...
          onRequestTrial={async () => {
            ...
            try {
              ...
              if (res.result === "OK" && res.granted > 0) {
                addToast(
                  `ì²´í—˜ ?°ì¼“ ${res.granted}ê°œê? ì§€ê¸‰ë˜?ˆìŠµ?ˆë‹¤! ?`,
                  "success",
                );
                ...
                addToast("?„ì¬??ì²´í—˜ ?°ì¼“??ë°›ì„ ???†ìŠµ?ˆë‹¤.", "error");
              }
            } catch (error: any) {
              const message =
