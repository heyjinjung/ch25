...
const LoadingView = () => (
  <section className="rounded-2xl border border-emerald-800/40 bg-slate-950/80 p-6">
    <p className="text-sm text-emerald-100">?¤ë¬¸??ë¶ˆëŸ¬?¤ëŠ” ì¤?..</p>
  </section>
);
...
const ErrorView = ({ onRetry, onBack }: { onRetry: () => void; onBack: () => void }) => (
  <section className="rounded-2xl border border-red-800/40 bg-slate-950/85 p-6">
    <p className="text-sm font-semibold text-red-100">?¤ë¬¸??ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??</p>
  ...
  if (q.question_type === "TEXT") {
    return (
      <textarea
        ...
        placeholder={q.helper_text || "?˜ê²¬???…ë ¥?˜ì„¸??}
...
const SurveyRunnerPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      addToast("?„ì‹œ ?€?¥ë˜?ˆìŠµ?ˆë‹¤.", "success");
    } catch {
      addToast("?€?¥ì— ?¤íŒ¨?ˆìŠµ?ˆë‹¤.", "error");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (code === "REQUIRED_ANSWERS_MISSING") {
        addToast("?„ìˆ˜ ë¬¸í•­??ëª¨ë‘ ?µë??´ì£¼?¸ìš”.", "error");
      } else if (code === "RESPONSE_COMPLETED") {
        addToast("?´ë? ?œì¶œ???¤ë¬¸?…ë‹ˆ??", "info");
        navigate("/surveys");
      } else {
        addToast("?œì¶œ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.", "error");
      }
    }
  };
  ...
  if (isCompleted) {
    return (
      <section className="flex min-h-[400px] flex-col items-center justify-center space-y-4 rounded-2xl border border-emerald-800/40 bg-slate-950/80 p-6 text-center">
        ...
        <div>
          <h2 className="text-xl font-bold text-emerald-100">?´ë? ì°¸ì—¬???¤ë¬¸?…ë‹ˆ??/h2>
          <p className="mt-1 text-sm text-slate-400">?Œì¤‘???˜ê²¬ ê°ì‚¬?©ë‹ˆ??<br />ë³´ìƒ?€ ?´ë? ì§€ê¸‰ë˜?ˆìŠµ?ˆë‹¤.</p>
        </div>
        <button
        ...
        {data.survey.questions.map((q, idx) => (
          <article key={q.id} className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              {q.is_required && <span className="rounded-full bg-red-900/50 px-2 py-1 text-[10px] font-semibold text-red-100">?„ìˆ˜</span>}
            </div>
