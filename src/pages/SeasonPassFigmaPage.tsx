...
const parseSeasonEndMs = (endDate?: string | null) => {
  ...
  // backend??date(YYYY-MM-DD) ?ïÌÉúÎ•??¥Î†§Ï§çÎãà??
...
const LevelCard: React.FC<{ variant: LevelCardVariant }> = ({ variant }) => {
  ...
  const displayName = user?.nickname || user?.external_id || "?åÎ†à?¥Ïñ¥";
    ...
    >
      ...
      <div className="mt-7">
        <div className="flex items-center justify-between text-[clamp(13px,2.8vw,14px)] text-white">
          <span>?àÎ≤® {derived.currentLevel}</span>
          <span>?àÎ≤® {derived.nextLevel}</span>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between text-[clamp(13px,2.8vw,14px)]">
          ...
          <span className="text-[#d2fd9c]">?§Ïùå ?àÎ≤®ÍπåÏ? {derived.remaining.toLocaleString()} XP ?®Ïùå</span>
        </div>
      </div>
      ...
      {season.isLoading ? (
        <p className="mt-6 text-[clamp(13px,2.8vw,14px)] text-white/65">?àÎ≤® ?ïÎ≥¥Î•?Î∂àÎü¨?§Îäî Ï§?..</p>
      ) : season.isError ? (
        <p className="mt-6 text-[clamp(13px,2.8vw,14px)] text-white/65">?àÎ≤® ?ïÎ≥¥Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??</p>
      ) : null}
    </section>
  );
};
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[clamp(15px,2.8vw,21px)] font-medium leading-[1.15] tracking-[-0.2px]" style={{ color: baseAccent }}>
                ÏßÄÎØºÏù¥?Ä ?®Íªò?òÎäî Í≤®Ïö∏ ?úÏ¶å ?®Ïä§
              </p>
...
const SeasonPassFigmaPage: React.FC = () => {
  ...
  const nudge = useMemo(() => {
    ...
    try {
      ...
      // storage ?ëÍ∑º ?§Ìå® ?úÏóê??Î∞∞ÎÑàÎ•??ÑÏòà ?ÑÏö∞ÏßÄ ?äÏïÑ Í≥ºÎèÑ???∏Ï∂ú??Î∞©Ï??©Îãà??
      return null;
    }

    const headline = kind === "season_end" ? "?úÏ¶å ÎßàÍ∞ê???ÑÎ∞ï?àÏñ¥?? : "?§Îäò ?ÑÏ†Å ÏßÑÌñâ??Ï±ôÍ≤®??;
    const body =
      remainingXp > 0
        ? `?§Ïùå Î≥¥ÏÉÅÍπåÏ? ${remainingXp.toLocaleString()} XPÎß???Î™®ÏúºÎ©??ºÏöî.`
        : "Ï°∞Í∏àÎß????òÎ©¥ ?§Ïùå Î≥¥ÏÉÅ??Î∞õÏùÑ ???àÏñ¥??";
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (remaining > 0 && progressPct >= 90) {
        ...
        addToast(
          `Í±∞Ïùò ???îÏñ¥?? ?§Ïùå Î≥¥ÏÉÅÍπåÏ? ${remaining.toLocaleString()} XPÎß???Î™®ÏúºÎ©??ºÏöî.`,
          "info"
        );
      }
    } catch {
      // sessionStorage ?ëÍ∑º Î∂àÍ? ?òÍ≤Ω?êÏÑú???†Ïä§?∏Î? ?§ÌÇµ?©Îãà??
    }
