...
const ConnectPage: React.FC = () => {
  ...
  const handleConnect = async () => {
    if (!initData) {
      addToast(
        "텔레그램 연결 정보를 찾을 수 없습니다. 텔레그램 앱 내에서 실행해 주세요.",
        "error",
      );
      return;
    }
    ...
    try {
      const response = await telegramApi.auth(
        initData,
      ...
      login(response.access_token, {
        ...response.user,
        cc_id:
          (response.user as any).cc_id ??
          response.user.external_id ??
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white p-6 pt-20">
      ...
      <p className="text-slate-400 text-center mb-10 max-w-xs">
        게임을 플레이하고 진행 상황을 안전하게 저장하려면 텔레그램 계정을 연결해
        주세요.
      </p>
      ...
      <button
        ...
        className={`w-full max-w-sm py-4 rounded-xl font-bold text-lg transition-all duration-300 transform active:scale-95 flex items-center justify-center gap-2 ${
          isConnecting
