...
const howToIcons: { title: string; icon: string; href?: string; to?: string }[] = [
  { title: "?®Ïî®?¥Ïö©?òÍ∏∞", icon: assets.iconWallet, href: "https://ccc-010.com" },
  { title: "Í∏àÍ≥†?úÎπÑ??, icon: assets.iconSecurity, to: "/vault" },
  { title: "?¨Ïù∏?∏Í≤å?ÑÌïòÍ∏?, icon: assets.iconGraph, to: randomGameLink },
  { title: "ÏπúÍµ¨Ï¥àÎ?", icon: assets.iconPeople, href: noticeLink },
];

const bento = [
  {
    title: "2x",
    description: "?¥Îîî?Ä??ÎπÑÍµêÎ∂àÍ????¨Ïù∏?∏ÏÑúÎπÑÏä§",
    highlight: true,
    icon: null,
  },
  {
    title: "Îπ†Î•¥Í≥??†ÏÜç??Í≥†Í∞ù?ëÎ? ?úÎπÑ??,
    description: "",
    icon: assets.bentoEfficiency,
    fallback: assets.vectorBar,
  },
  {
    title: "?àÏ†Ñ?òÍ≥† Îπ†Î•∏ ?òÏ†Ñ",
...
const Events: React.FC = () => (
  <section className="bg-white flex flex-col items-center px-[40px] py-[20px] w-full">
    ...
    >
      <img src={assets.headerImage} alt="?¥Î≤§??Î∞∞ÎÑà" className="absolute inset-0 h-full w-full object-contain" />
    </a>
  </section>
);
      ...
      {[1, 2].map((key) => (
        ...
        >
          ...
          <p className="text-[18px] lg:text-[20px] font-medium leading-[1.15] text-left" style={{ color: deepOlive }}>
            {key === 1 ? "?†ÎÖÑ?¥Î≤§???¥Í∏àÍ≥??¥Í∏∞" : "Í∏àÍ≥†Î®∏Îãà ?ÅÎ¶Ω?¥Î≤§??}
          </p>
        </a>
      ))}
    </div>
      ...
      <div className="grid w-full max-w-[900px] gap-[20px] sm:grid-cols-2">
        {bento.slice(2).map((item) => (
          ...
          >
            <div className="flex flex-col items-center gap-[30px] text-center text-[#394508] w-full">
              {item.icon && (
                <div className="relative" style={{ height: item.title === "Í≥†Í∞ùÎßåÏ°±??1?? ? 67.5 : 112, width: item.title === "Í≥†Í∞ùÎßåÏ°±??1?? ? 199.281 : 124.31 }}>
                  <img
                    src={item.icon}
                    alt={item.title}
                    className="h-full w-full object-contain"
