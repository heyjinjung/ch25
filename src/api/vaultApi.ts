...
const mapWithdrawalErrorMessage = (detail: unknown): string | null => {
  ...
  switch (detail) {
    case "DEPOSIT_REQUIRED_TODAY":
      return "?¤ëŠ˜ ?…ê¸ˆ(ì¶©ì „) ?´ì—­???ˆì–´??ì¶œê¸ˆ ? ì²­??ê°€?¥í•©?ˆë‹¤.";
    case "NO_DEPOSIT_RECORD_TODAY":
      return "?¤ëŠ˜ ?…ê¸ˆ(ì¶©ì „) ?´ì—­???•ì¸?˜ì? ?Šì•„ ì¶œê¸ˆ ? ì²­??ë¶ˆê??¥í•©?ˆë‹¤.";
    case "MIN_PLAY_COUNT_30_REQUIRED":
        return "ìµœê·¼ 3???´ë‚´ ê²Œì„ ?Œë ˆ??30??ì¡°ê±´??ë§Œì¡±?´ì•¼ ?©ë‹ˆ??";
    case "MIN_DAILY_SPEND_10000_REQUIRED":
        return "?¤ëŠ˜ ê¸ˆê³  ?¬ìš©?¡ì´ 10,000???´ìƒ?´ì–´???©ë‹ˆ??";
    case "NO_DEPOSIT_HISTORY":
        return "?…ê¸ˆ ?´ë ¥???†ëŠ” ê³„ì •?€ ì¶œê¸ˆ?????†ìŠµ?ˆë‹¤.";
    case "DEPOSIT_REQUIRED_TODAY_SYNC":
        return "?¤ëŠ˜ ?…ê¸ˆ(ì¶©ì „) ê¸°ë¡???•ì¸?˜ì? ?Šì•˜?µë‹ˆ??";
    default:
      return null;
  }
};
...
export const requestWithdrawal = async (amount: number): Promise<{ success: boolean; message: string }> => {
  try {
    ...
    return { success: true, message: "ì¶œê¸ˆ ? ì²­???„ë£Œ?˜ì—ˆ?µë‹ˆ??" };
  } catch (err: any) {
    // Handle specific errors like 'insufficient_funds', 'daily_limit', etc.
    const detail = err.response?.data?.detail;
    const msg = mapWithdrawalErrorMessage(detail) ?? detail ?? "? ì²­ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.";
