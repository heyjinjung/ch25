...
const formatRewardLabel = (rewardType?: string, rewardAmount?: number, rewardLabel?: string) => {
  ...
  if (type === "POINT" || type === "CC_POINT") return `�ݰ� ${formattedAmount ?? ""}`.trim();
  if (type === "GAME_XP" || type === "XP") return `���� XP ${formattedAmount ?? ""}`.trim();
  if (type === "DIAMOND") return `���̾� ${formattedAmount ?? ""}`.trim();
  if (type.includes("TICKET")) return `Ƽ�� ${formattedAmount ?? ""}`.trim();
  if (type.includes("GIFTICON")) return `����Ƽ�� ${formattedAmount ?? ""}`.trim();
