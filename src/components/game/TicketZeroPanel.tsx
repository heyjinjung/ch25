...
const TicketZeroPanel: React.FC<TicketZeroPanelProps> = ({ tokenType }) => {
  ...
  const title = value.title || `${tokenType}ê°€ ë¶€ì¡±í•´??;
  const body = value.body || "?´ìš©??ì£¼ì…”??ê°ì‚¬?©ë‹ˆ?? ì§€ê¸??´ìš©?˜ì‹œë©?ë°”ë¡œ ?´ì–´???Œë ˆ??ê°€?¥í•©?ˆë‹¤.";

  const primaryLabel = value.primaryCta?.label || value.primary_cta_label || "?¨ì”¨ ??;
  const primaryUrl = value.primaryCta?.url || value.primary_cta_url || "https://ccc-010.com";

  const secondaryLabel = value.secondaryCta?.label || value.secondary_cta_label || "?¤ì¥ ?”ë ˆ ë¬¸ì˜";
