...
export const SeasonPassRewardsAndTasks: React.FC = () => {
  ...
  const tasks: TaskRow[] = useMemo(() => {
    ...
    rows.push({
      title: "?¤ëŠ˜ ?¤íƒ¬??,
      description: "?¤ëŠ˜ ?¤íƒ¬?„ë? ?„ë£Œ?˜ë©´ ë³´ìƒ??ì§„í–‰?©ë‹ˆ??",
      status: stamped == null ? "?°ì´???†ìŒ" : stamped ? "?„ë£Œ" : "ë¯¸ì™„ë£?,
    ...
    if (win) {
      ...
      rows.push({
        title: "ê²¨ìš¸ ê²Œì„ ?¹ë¦¬",
        description: "?´ë? ?¹ë¦¬ ?„ì  ëª©í‘œ ?¬ì„±",
        status: `${done.toLocaleString()} / ${denom.toLocaleString()} (?¨ì? ${Math.max(0, win.remaining ?? 0).toLocaleString()})`,
      ...
      rows.push({
        title: "ê²¨ìš¸ ê²Œì„ ?¹ë¦¬",
        description: "?´ë? ?¹ë¦¬ ?„ì  ëª©í‘œ ?¬ì„±",
        status: internalWins.isLoading ? "ë¶ˆëŸ¬?¤ëŠ” ì¤? : "?°ì´???†ìŒ",
        progressPct: null,
      });
    }
    ...
    rows.push({
      title: "ë£°ë › ê²½í’ˆ",
      description: "?¨ì? ë£°ë › ?´ìš© ?Ÿìˆ˜",
      status: r == null ? (roulette.isLoading ? "ë¶ˆëŸ¬?¤ëŠ” ì¤? : "?°ì´???†ìŒ") : `${r}??,
      progressPct: null,
    });
    ...
    rows.push({
      title: "?ˆë²¨ ì£¼ì‚¬??,
      description: "?¨ì? ì£¼ì‚¬???´ìš© ?Ÿìˆ˜",
      status: d == null ? (dice.isLoading ? "ë¶ˆëŸ¬?¤ëŠ” ì¤? : "?°ì´???†ìŒ") : `${d}??,
      progressPct: null,
    });
    ...
    rows.push({
      title: "?œë¤ ë³µê¶Œ",
      description: "?¨ì? ë³µê¶Œ ?´ìš© ?°ì¼“",
      status: l == null ? (lottery.isLoading ? "ë¶ˆëŸ¬?¤ëŠ” ì¤? : "?°ì´???†ìŒ") : `${l}??,
      progressPct: null,
    });
    ...
    <div className="w-full">
      <div className="mx-auto flex w-full max-w-[980px] flex-col gap-5 lg:flex-row">
        <div className="w-full lg:w-3/5">
          <PanelShell title="?ˆë²¨ ë³´ìƒ">
            {season.isLoading ? (
              <p className="text-[clamp(13px,2.8vw,14px)] text-white/65">ë¶ˆëŸ¬?¤ëŠ” ì¤?..</p>
            ) : season.isError ? (
              <p className="text-[clamp(13px,2.8vw,14px)] text-white/65">ë³´ìƒ ?•ë³´ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??</p>
            ) : (
              <div className="flex flex-col gap-3">
                {rewards.map((reward) => {
                  ...
                  const isManualAdmin =
                    /CC\s*(?¬ì¸??ì½”ì¸|POINT|COIN)/i.test(reward.reward_label) ||
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <p className={`text-[clamp(14px,2.8vw,16px)] font-semibold ${labelTone}`}>?ˆë²¨ {reward.level}</p>
                            {reward.isCurrent && (
                        ...
                        <div className="shrink-0 text-right">
                          <p className="text-[clamp(13px,2.6vw,14px)] text-white/65">?„ìš” XP</p>
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                          {reward.is_claimed ? (
                            ...
                            <span className="text-[clamp(13px,2.6vw,14px)] text-amber-200">?´ê¸ˆ ??ê´€ë¦¬ì ?•ì¸ ??ì§€ê¸?/span>
                          ) : reward.auto_claim ? (
                            <span className="text-[clamp(13px,2.6vw,14px)] text-cc-lime">?ë™ ì§€ê¸??€??/span>
                          ) : reward.isUnlocked ? (
                            <span className="text-[clamp(13px,2.6vw,14px)] text-white/75">?ë“ ê°€??/span>
                          ) : (
                            <span className="text-[clamp(13px,2.6vw,14px)] text-white/55">? ê¸ˆ</span>
                          )}
                        </div>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                const msg = res.message || `${res.level}?ˆë²¨ ë³´ìƒ ì§€ê¸??„ë£Œ`;
                                addToast(msg, "success");
                              } catch {
                                addToast("ë³´ìƒ ì§€ê¸‰ì— ?¤íŒ¨?ˆìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„??ì£¼ì„¸??", "error");
                              } finally {
                                setPendingClaimLevel(null);
                              }
                            }}
                          >
                            {isPending ? "ì§€ê¸?ì¤?.." : "ë³´ìƒ ë°›ê¸°"}
                          </button>
                        ) : null}

                        {isManualAdmin && !reward.is_claimed && reward.isUnlocked ? (
                          <p className="text-[clamp(12px,2.5vw,13px)] text-amber-200 font-medium">ê´€ë¦¬ì ?•ì¸ ???¸ë? ì§€ê¸‰ë©?ˆë‹¤.</p>
                        ) : null}
                      </div>
                    </motion.div>
                  );
