...
const TodayMissionCard: React.FC<TodayMissionCardProps> = ({ data }) => {
    ...
    const handleClaim = async () => {
        ...
        if (res.success) {
            const rewardMsg = res.amount ? `${res.amount}${res.reward_type || ""}???òÎ†π?àÏäµ?àÎã§!` : "Î≥¥ÏÉÅ???òÎ†π?àÏäµ?àÎã§!";
            addToast(rewardMsg, "success");
        } else {
            addToast(res.message || "Î≥¥ÏÉÅ ?òÎ†π???§Ìå®?àÏäµ?àÎã§.", "error");
        }
        setIsClaiming(false);
    };
            ...
            <div className="relative p-6">
                <div className="flex justify-between items-start mb-6">
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="text-sm font-medium text-white/40">{mission.description || "ÏßÄÍ∏?Î∞îÎ°ú ?ÑÏ†Ñ?òÍ≥† Î≥¥ÏÉÅ??Î∞õÏúº?∏Ïöî."}</p>
                    </div>
