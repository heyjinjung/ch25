...
interface NewUserWelcomeModalProps {
    onClose: () => void;
    onClaimSuccess?: () => void;  // Called only when "Î∞õÍ∏∞" succeeds
}
    ...
    const handleClaim = async () => {
        ...
        try {
            ...
            if (!result?.success) {
                ...
                notification("error");
                addToast(`?∞Ïª¥ Î≥¥ÏÉÅ ÏßÄÍ∏âÏóê ?§Ìå®?àÏäµ?àÎã§. ${result?.reason || '?†Ïãú ???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.'}`, "error");
                return;
            }
            ...
            addToast("?ïÏ∞© ÏßÄ?êÍ∏à??ÏßÄÍ∏âÎêò?àÏäµ?àÎã§.", "success");
            ...
            notification("error");
            const errorMsg = (error as any)?.response?.data?.detail || (error as any)?.message || "?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§. ?†Ïãú ???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.";
            ...
            return (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
                    <div className="w-full max-w-[340px] rounded-[32px] border border-white/10 bg-zinc-950 p-10 text-center shadow-2xl">
                        ...
                        <p className="text-white/60 font-bold tracking-tight">ÏµúÍ≥†???úÌÉù??Ï§ÄÎπ?Ï§?..</p>
                    </div>
                </div>
            );
        }
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="relative pt-10 pb-4 px-6 text-center">
                        ...
                        <p className="text-zinc-500 text-xs font-medium tracking-tight">
                            Î∞îÎ°ú Í≤åÏûÑ???úÏûë?òÏã§ ???àÎèÑÎ°?Ï§ÄÎπÑÌñà?µÎãà??
                        </p>
                    </div>
                    ...
                    <div className="px-6 pb-6">
                        <div className="grid grid-cols-2 gap-2 mb-6">
                            ...
                            <div className="relative group p-4 rounded-2xl bg-white/[0.03] border border-white/5 flex flex-col items-center justify-center overflow-hidden aspect-[4/5]">
                                ...
                                <div className="relative z-10 text-[12px] font-bold text-zinc-500 mb-1 whitespace-nowrap">?ïÏ∞© ÏßÄ?êÍ∏à</div>
                            ...
                            <div className="relative group p-4 rounded-2xl bg-white/[0.03] border border-white/5 flex flex-col items-center justify-center overflow-hidden aspect-[4/5]">
                                ...
                                <div className="relative z-10 text-[12px] font-bold text-zinc-500 mb-1 whitespace-nowrap">Î£∞Î†õ ?∞Ïºì</div>
                                <div className="relative z-10 text-xl font-black text-white tabular-nums">
                        ...
                        {!hasClaimed ? (
                            ...
                            <div className="space-y-2 animate-fadeIn">
                                <div className="flex flex-col items-center justify-center py-2 text-emerald-400 gap-1 animate-bounce-subtle">
                                    ...
                                    <span className="font-black text-sm">ÏßÄÍ∏??ÑÎ£å!</span>
                                </div>
