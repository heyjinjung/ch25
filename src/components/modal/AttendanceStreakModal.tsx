...
const RefreshCw = ({ className }: { className?: string }) => (
    <svg className={clsx("w-6 h-6", className)} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
const AttendanceStreakModal: React.FC<AttendanceStreakModalProps> = ({ onClose, onClaim, currentStreak, claimableDay, rules }) => {
    ...
    const getRewardIcon = (day: number, grants: Reward[]) => {
        const isLastDay = day === 7;
        const sizeClass = isLastDay ? "w-14 h-14" : "w-10 h-10";

        if (grants.length > 1) {
            return <img src="/assets/lottery/icon_gift.png" alt="Gift" className={clsx(sizeClass, "object-contain")} />;
        }
        const g = grants[0];
        if (!g) return <Star className={clsx(sizeClass, "text-white/20")} />;

        if (g.token_type === "ROULETTE_COIN") return <span className={isLastDay ? "text-4xl" : "text-3xl"}>?éØ</span>;
        if (g.token_type === "DICE_TOKEN") return <span className={isLastDay ? "text-4xl" : "text-3xl"}>?é≤</span>;
        if (g.item_type === "DIAMOND" || g.token_type === "DIAMOND") {
            return <img src="/assets/icon_diamond.png" alt="Diamond" className={clsx(sizeClass, "object-contain")} />;
        }
        return <img src="/assets/lottery/icon_gift.png" alt="Reward" className={clsx(sizeClass, "object-contain")} />;
    };
    ...
    const handleClaim = async () => {
        ...
        if (claimableDay && onClaim) {
            ...
            setIsClaiming(false);
            if (success) {
                // Keep modal open or close? Current logic closes it.
                // onSuccess ?∞Ï∂ú???àÏúºÎ©?Ï¢ãÍ≤†ÏßÄÎß??¥Î≤à??Í∞ÑÎã®??Ï¢ÖÎ£å.
                onClose();
            }
        } else {
            onClose();
                ...
                <div className="relative p-6 sm:p-8">
                    ...
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            ...
                            <h2 className="text-3xl font-black text-white tracking-tight">
                                {isClaimable ? "?§Îäò??Î≥¥ÏÉÅ ?ÑÏ∞©!" : `?î• ${currentStreak}??Ï∂úÏÑù Ï§?`}
                            </h2>
                        </div>
                        <button
                            type="button"
                    ...
                    <div className="relative mb-6">
                        ...
                        <div className="flex justify-between mt-2 px-1">
                            ...
                            <span className="text-[10px] font-black text-amber-500">{currentStreak}/7??/span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="p-5 rounded-3xl bg-white/5 border border-white/10">
                            <div className="flex justify-between items-center mb-4">
                                <p className="text-xs font-black text-white/40 uppercase tracking-widest">Ï∂úÏÑù?ÑÌô©</p>
                                {/* Active Now indicator removed */}
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="text-4xl font-black text-white tabular-nums">{currentStreak}</div>
                                <div className="flex-1">
                                    <p className="text-sm font-bold text-white/80">???∞ÏÜç ?åÎ†à??Ï§?</p>
                                    <p className="text-[11px] font-medium text-white/40">Îß§Ïùº ?êÏ†ï(00:00) Í∏∞Ï? Í∞±Ïã†</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-black text-amber-500">Next Target</p>
                                    <p className="text-lg font-black text-white">Day {currentStreak + 1}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            {isClaiming ? <RefreshCw className="animate-spin" /> :
                                isClaimable ? "?éÅ ?§Îäò??Î≥¥ÏÉÅ Î∞õÍ∏∞" :
                                    currentStreak === 0 ? "Í≤åÏûÑ ?òÍ≥† Î≥¥ÏÉÅ ?úÏûë?òÍ∏∞ ?éÆ" : "?¥Ïùº ?§Ïãú ÎßåÎÇò??"}
                        </Button>
