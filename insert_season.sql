...
SELECT id, 1, 0, 'ROULETTE_TICKET', 3, 1 FROM season_pass_config WHERE is_active = 1 AND season_name = '연말 브릿지 시즌 (12/25-1/1)' ORDER BY id DESC LIMIT 1;

INSERT INTO season_pass_level (season_id, level, required_xp, reward_type, reward_amount, auto_claim) 
SELECT id, 2, 50, 'DICE_TICKET', 3, 1 FROM season_pass_config WHERE is_active = 1 AND season_name = '연말 브릿지 시즌 (12/25-1/1)' ORDER BY id DESC LIMIT 1;
...
INSERT INTO season_pass_level (season_id, level, required_xp, reward_type, reward_amount, auto_claim) 
SELECT id, 8, 800, 'CC_POINT', 30000, 0 FROM season_pass_config WHERE is_active = 1 AND season_name = '연말 브릿지 시즌 (12/25-1/1)' ORDER BY id DESC LIMIT 1;
