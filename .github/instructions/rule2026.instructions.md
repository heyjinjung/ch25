...
## 7. Brainstorming/설계 스킬 적용 

- 모든 창의적/구조적 작업(기능, 컴포넌트, 아키텍처, 동작/기능 변경 등) 전 반드시 아래 Brainstorming 스킬을 선행한다.
- Brainstorming 스킬 프로세스(이해→질문→비기능 요구→이해락→대안→설계→결정로그→문서화→구현준비)를 그대로 따른다.
- 설계/이해락(Understanding Lock) 전에는 절대 구현/코드/동작 변경 금지.
- 한 번에 한 질문만, 가정/비기능 요구/대안/결정로그 명시.
- 설계가 고위험/고임팩트일 경우 multi-agent-brainstorming skill로 핸드오프.
- 설계/결정로그/가정/최종안은 Markdown 등 내구성 있는 포맷으로 기록하고, 프로젝트 표준 워크플로에 따라 문서화 및 이력 관리.
- 기존 SoT/learned_ 규칙과 충돌 시, 항상 최신 learned_와 인덱스 0번 섹션을 우선한다.
