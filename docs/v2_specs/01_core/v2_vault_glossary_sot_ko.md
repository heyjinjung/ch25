...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 환전(출금) | Withdraw | `vault_withdrawal_request` | **[조건]**<br>1) 당일 `cc_deposit` 1만 이상 (순증)<br>2) 오늘 사용 금액(상점 이용) 조건 충족<br>3) 최근 3일 게임 플레이 조건 충족<br>4) 회차별 최소 금액 조건 충족<br>(정책 상세: [강력한 금고 정책 SoT](v2_strict_vault_policy_sot_ko.md#31-출금-자격-withdrawal-eligibility)) | **[회차별 최소 금액]**<br>1회: 1만 / 2회: 1만<br>3회: 3만 / 4회: 5만 |
| 당일 금고 사용 | `vault_spent_today` | `user.vault_spent_today` | 당일 금고 사용 누적액. 출금 조건(당일 1만원 사용) 판단용. | 일일 리셋 | 1일 1회 리셋 필요 |
...
- WHALE은 플레이/소비 조건이 면제(0/0)되며, **당일 실질 입금 조건은 유지**된다.
