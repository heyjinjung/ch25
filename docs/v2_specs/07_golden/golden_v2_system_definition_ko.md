...
| Admin Queue | `golden:v2:admin:queue` | 승인 대기열 실시간 갱신(Human-in-the-loop) |
| Config | `golden:v2:config:updates` | 운영 설정/상태 변경 즉시 반영(골든아워 포함) |
...
| 구분 | 엔드포인트 | 대상 |
| :--- | :--- | :--- |
| Admin WebSocket | `/api/admin/ws/golden/events` | 어드민 대시보드 |
