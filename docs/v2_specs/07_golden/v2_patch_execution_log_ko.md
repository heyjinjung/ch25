...
| Task | 설명 | 예상 시간 |
|------|------|----------|
| **ROI 대시보드** | ROI 시각화 차트 | 1시간 |
| **잠재고객 관리 페이지** | 미가입 잠재고객 목록 + 알림 | 1시간 |
| **골든아워 설정 UI** | 골든아워 수동/자동 토글 | 30분 |
...
| 방식 | 설명 | 장점 | 구현 상태 |
|------|------|------|----------|
| **방식 A: 유저 직접 입력** | 가입 시 CC 닉네임 입력 필드 제공 | 빠른 매칭, 유저 주도 | ⏳ 프론트 미구현 |
| **방식 B: 어드민 수동 매칭** | 관리자가 잠재고객 목록에서 V2User 선택 | 정확한 매칭, 검증 가능 | ⏳ 프론트 미구현 |
...
| **프론트엔드** | ⏳ 미구현 | 매칭 결과 UI 없음 (유저는 모르는 사이에 매칭됨) |
