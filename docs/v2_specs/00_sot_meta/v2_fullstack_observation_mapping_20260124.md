...
| 12 | TicketZero Flow & Cooldown | Medium | @owner | 티켓 제도(쿨다운/지급 제한) 동작 확인 | backend: `pytest -q tests/v2_tests/phase2_core/test_ticket_zero.py` | frontend: `cypress: tests/e2e/ticket_zero.spec.ts` | ticket_zero logs, vault_ledger, response bodies, screenshots | 티켓 지급 조건/쿨다운이 정책대로 작동 | todo |
| 13 | Hybrid Routing (Admin V1/V2 mix) | High | @owner | `/admin/` 경로에 V2 컴포넌트(CC 입금/금고) 통합 및 렌더링 확인 | n/a | frontend: `npx cypress run --spec "cypress/e2e/admin_nav_smoke.cy.ts"` | browser address bar logs, console errors, network panel | V1 사이드바에서 V2 페이지 이동 시 세션 유지 및 컴포넌트 정상 출력 | passed (evidence: `artifacts/20260124/routing_fix_summary.txt`) |
| 14 | Admin API Token Proxy Fix | High | @owner | `/admin/` 경로에서 호출되는 V2 API의 토큰 첨부 로직 검증 | n/a | frontend: network monitor (Chrome devtools / HAR) | request headers (Authorization: Bearer <admin_token>) | `/admin/economy/...` 호출 시 유저 토큰이 아닌 관리자 토큰이 정확히 실림 | passed (evidence: `v2_verification_test_logs_20260124.md` CASE 7.1) |
