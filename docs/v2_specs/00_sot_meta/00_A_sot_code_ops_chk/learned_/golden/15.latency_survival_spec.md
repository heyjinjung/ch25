...
- **Service**: `app/v2/services/latency_survival_service.py`
    - **Provisional Grant**: TX ID 중복/Rate Limit 체크 후 즉시 보상(룰렛티켓 5장) 지급.
...
### 2.3 User Frontend (❌ 미구현 - Critical Gap)
- **Status**: 현재 유저가 증거를 제출할 수 있는 UI가 존재하지 않습니다.
- **Impact**: 백엔드와 어드민 기능이 준비되었으나, 유저 접근 불가로 기능 사용 불가 상태.
...
| 항목 | 값/정책 | 비고 |
| :--- | :--- | :--- |
| **선지급 보상** | `ROULETTE_TICKET` x 5 | 설정값 (`PROVISIONAL_REWARD_AMOUNT`) |
...
sequenceDiagram
    ...
    alt 중복/어뷰징 감지 (Rate Limit)
        ...
        API->>Inventory: Provisional Grant (5 Tickets)
        API-->>Frontend: 200 OK
        Frontend->>User: "접수 완료! 티켓 5장 선지급되었습니다."
    end
```
...
- **Feedback**:
    - 성공 시: "신고 접수 완료! 룰렛 티켓 5장이 선지급되었습니다." (Confetti 효과).
