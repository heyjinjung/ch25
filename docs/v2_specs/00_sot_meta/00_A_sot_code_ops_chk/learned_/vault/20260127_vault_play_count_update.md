...
- **ID 불일치**: `request_withdrawal` API에서 V2 유저 ID를 호환 테이블인 `user`(legacy) 조회에 직접 사용하여, ID 체계가 다른 유저의 경우 `USER_NOT_FOUND` 오류가 발생하거나 잘못된 유저 정보를 참조함. (**V2 정책 반영**: 유저 식별 SoT는 `v2_user(cc_id)`이며, `user.id`는 호환 목적)
...
- **ID 정합성 확보**:
  - `request_withdrawal` 시작 시 `V2UserService.ensure_legacy_user_id()`를 호출하여 V2 유저 ID에 매칭되는 `user.id`(legacy/호환)를 획득하도록 수정. (**V2 정책 반영**)
