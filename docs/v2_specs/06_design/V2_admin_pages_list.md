...
| **메시지 발송** | `/v2/admin/marketing/messages` | **Real** | **인박스(INBOX) 메시지 전송만 지원**(PUSH 제거), 세그먼트 타겟팅(세그먼트 통계 기반), 발송 이력 |
...
### 3-6. 마케팅 메시지/설문 정합성 업데이트 (2026-01-20)
- **PUSH 기능 제거**: 어드민 메시지 발송은 **INBOX-only**로 동작(요청 payload 및 백엔드 저장 채널 정규화).
- **세그먼트 옵션 하드코딩 제거**: 메시지 타겟 세그먼트는 세그먼트 통계 API 기반으로 동적 렌더링.
- **유저 설문 응답 API v2 통일**: 프론트 유저 설문 플로우는 `/api/v2/surveys/*`로 호출(레거시 `/api/surveys/*`와 동일 동작의 v2 별칭 제공).
