...
### 02-04 - LEVEL/ADMIN: 레벨 XP 가산/감산 404 (운영 배포 누락) ✅
...
| 대상 기능 | 어드민 레벨/XP 가산·감산 |
| HTTP Status | 404 (Not Found) |
| 영향 범위 | 운영 어드민 레벨 관리 화면 |
| 재현 빈도 | 항상 |

**근본 원인 (증거 기반)**
- 운영 Nginx access.log에서 `POST /api/v2/admin/users/level/adjust`가 404로 기록됨.
- 운영 컨테이너 로드 라우트에는 해당 경로가 존재하므로, **배포 반영 이전 버전 또는 라우팅 미반영 상태**로 판단.

**해결 방법**
- 운영에 최신 코드 배포.
- 배포 후 `/api/v2/admin/users/level/adjust` 200 응답 확인.

**검증 방법**
- 운영에서 `POST /api/v2/admin/users/level/adjust`가 200으로 응답하는지 확인.
- 조정 전/후 `GET /api/v2/admin/users/level`의 XP 값이 변경되는지 확인.
