...
### 3.5 위기 레이더 (Crisis Radar) 상태 조회
- Endpoint: `GET /api/v2/admin/ops/status`
- Response:
```json
{
  "system_status": "OK",
...
### 3.6 유저 상세 및 개입 플레이북 (User CRM)
- Endpoint: `GET /api/v2/admin/users/{user_id}`
- Response: `AdminUserDetailDto` (스키마 참조)

### 3.7 Ops 실행 결과 저장/조회
- Endpoint: `POST /api/v2/admin/ops/tasks/{task_id}/execution-result`
- Endpoint: `GET /api/v2/admin/ops/tasks/{task_id}/execution-result`
...
## 5. 변경 이력
- v1.4 (2026-01-19, GitHub Copilot): 위기 레이더(Crisis Radar) 및 유저 CRM 엔드포인트 추가, 경로 prefix 정정 (/api/v2/admin/)
