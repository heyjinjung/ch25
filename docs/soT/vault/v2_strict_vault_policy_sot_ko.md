...
[최종 검토일: 2026-01-26]
[정책 최신화 필요 여부: 🟡] 🟡 [정합성 검토 필요] 정책(합산 금지)은 유효하나, 일부 구현에서 `locked+available` 합산 흔적이 보여(대시보드/집계) 정책 준수 여부 점검 필요.
...
#### 3.1.0 세그먼트별 출금 조건 (Finalized)
출금 조건의 **게임 플레이 횟수**와 **오늘 사용 금액**은 유저 세그먼트에 따라 아래와 같이 강제됩니다.

| 세그먼트 | 최근 3일 게임 플레이 횟수 | 오늘 사용 금액(상점 이용) | 비고 |
| :--- | :--- | :--- | :--- |
| **NEW** | **5회** | **0원** | 가입 7일 이내 (신규 혜택 강조) |
| **COMMON** | **15회** | **5,000원** | 당일 1만 이상 입금 조건 유지 |
| **VIP** | **10회** | **0원** | **당일 10만 이상 입금 시** |
| **WHALE** | **0회** | **0원** | **당일 10만 이상 입금 시** (면제) |
| **AT_RISK** | **30회** | **10,000원** | 위험 유저 (완화됨) |

- **최근 3일 기준**: Asia/Seoul, 오전 9시 리셋(Operational Day) 정책을 따른다.
- WHALE은 플레이/소비 조건이 면제(0/0)되나, **당일 실질 입금 조건(10만+)은 유지**된다.
- **입금 조건**: COMMON 이하는 당일 1만 이상, VIP 이상은 당일 10만 이상 실질 입금 필요.
- **오늘 사용 금액**: 상점(`v2_shop_order`)에서 사용된 금액만 합산된다.
...
| **ADMIN_FORCED** | **어드민 수동 설정** | **`True/False`** | 어드민이 유저별로 제재 상태를 직접 관리 가능 |
...
- v2.1 (2026-02-02, GitHub Copilot): NEW 세그먼트 출금 조건 추가
