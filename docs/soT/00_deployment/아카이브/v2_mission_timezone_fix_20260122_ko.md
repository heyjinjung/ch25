...
```mermaid
graph LR
    A[User Login] --> B{API Layer Check}
    B -->|Midnight KST| C[LOGIN mission updated]
    C --> D{MissionService Check}
    D -->|9AM KST| E[Missions reset]
    
    F[User at 2AM KST] --> G[LOGIN = Today]
    G --> H[Missions = Yesterday]
    H --> I[❌ Button Disabled]
```
...
```mermaid
graph LR
    A[User Login] --> B[MissionService]
    B --> C{Operational Day Check}
    C -->|Before 9AM| D[Use Yesterday's Date]
    C -->|After 9AM| E[Use Today's Date]
    D --> F[Consistent reset_date]
    E --> F
    F --> G[✅ Button Active]
```
...
### Phase 1: Backend Core Logic ✅ **COMPLETED** (2026-01-22 15:35 KST)

- [x] **`mission_service.py`**
  - [x] Add `ACTION_TYPE_ALIASES` constant (Lines 16-23)
  - [x] Implement `_normalize_action_type()` method (Lines 25-45)
  - [x] Update `update_progress()` to use normalized actions (Line 575)
  - [ ] Add unit tests for normalization logic (TODO)

- [x] **`mission.py` (API Route)**
  - [x] Replace direct `kst.date()` with `service._operational_play_date()` (Line 82)
  - [x] Remove timezone calculation from API layer (Lines 76-82)
  - [ ] Add integration test for 12AM-9AM edge case (TODO)

**Implementation Summary**:
- ✅ Telegram mission compatibility issue FIXED (JOIN_CHANNEL ↔ SUBSCRIBE_CHANNEL)
- ✅ Split-brain reset logic UNIFIED (9AM KST across all components)
- ✅ Backward compatibility maintained (legacy action types still work)

### Phase 2: Frontend Integration ✅ (Priority 2)

- [ ] **`MissionsPage.tsx`**
  - [ ] Replace mock data with `useV2Missions` hook
  - [ ] Implement `handleClaim` with mutation
  - [ ] Add loading/error states
  - [ ] Dynamic progress bar rendering

- [ ] **`useV2ClaimMission`**
  - [ ] Add `onSuccess` refetch callback
  - [ ] Implement optimistic UI update

### Phase 3: Verification ✅ (Priority 3)

- [ ] **Manual Test Scenarios**
  - [ ] Login at 8:59 AM KST → Verify yesterday's missions
  - [ ] Login at 9:01 AM KST → Verify today's missions
  - [ ] Telegram channel join → Verify reward granted
  - [ ] Claim reward → Verify UI updates immediately

- [ ] **Automated Tests**
  - [ ] `test_mission_operational_day_reset()`
  - [ ] `test_action_type_normalization()`
  - [ ] `test_telegram_mission_integration()`
...
| Risk | Mitigation |
|------|------------|
| Breaking existing missions | Add normalization as additive (backward compatible) |
| Timezone confusion in logs | Add explicit `timezone` field to all event logs |
| Race condition on claim | Use existing `with_for_update()` row lock |
| Frontend state desync | Implement optimistic updates + refetch on error |

---

## 5. Rollout Plan

1. **Backend Deploy** (No user impact)
   - Deploy normalization logic
   - Monitor logs for action type matches

2. **Frontend Deploy** (Gradual)
   - Deploy real-time binding
   - Monitor claim success rate

3. **Validation** (24h window)
   - Verify 9AM reset behavior
   - Check Telegram mission completion rate
...
- `app/services/mission_service.py` (Lines 16-45: ACTION_TYPE_ALIASES & Normalization)
- `app/api/routes/mission.py` (Lines 76-96: Operational Day Login Check)

---

## 8. 변경 이력
- v1.0 (2026-01-22 14:00): 최초 작성 (기술 설계)
- v1.1 (2026-01-22 15:40): Phase 1 완료 반영 (백엔드 구현)
