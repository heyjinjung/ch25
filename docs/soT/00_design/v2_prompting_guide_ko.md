...
[최종 검토일: 2026-01-26]
[정책 최신화 필요 여부: 🟢] 🟢 [정합] 금고/티켓 한글 라벨, No-scroll 등 UI 제약은 Vault/운영 문서와 충돌 없음.
...
### ⑤ 배경 워터마크 브랜딩 (Background Watermark Branding)
게임 페이지의 깊이감과 신뢰도를 위해 배경 워터마크를 표준 적용합니다.
> "화면 중앙(메인 콘텐츠 뒤)에 `CC` 로고를 가로 `300px` 크기로 배치해. 투명도는 `opacity-5`에서 `opacity-8` 사이로 아주 은은하게 처리해서 배경 워터마크처럼 보이게 해."
