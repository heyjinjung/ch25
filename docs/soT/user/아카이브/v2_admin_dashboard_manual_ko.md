...
### 1. 모니터링 & 분석 (Monitoring & Analytics)
> **PATH**: `/admin/control`, `/admin/analytics`, `/admin/system`
*   **용도**: 전체 시스템 관제, 통합 데이터 분석, 보안 및 반입 관리를 수행합니다.
*   **하위 메뉴**:
    *   **통합 관제 센터**: 전체 현황, 실시간 중계, 이탈 위험 모니터링 (`/admin/control`)
    *   **지표 및 인사이트**: 리텐션, 수익/지출 등 정밀 분석 (`/admin/analytics`)
    *   **시스템 보안 및 CSV**: 비상 정지, 지연 승인, CSV 반입, 감사 로그 통합 관리 (`/admin/system`)
...
*   **선택 헤더**:
    ...
    *   `세그먼트`: 지정하려는 그룹 (VIP, WHALE, AT_RISK, COMMON 중 선택. **파일 내 값이 있을 경우 자동 분류보다 우선함**)

##### **C. 자동 세그먼트 분류 기준 (Classification Logic)**
별도의 세그먼트를 지정하지 않고 임포트할 경우, 시스템은 다음 기준에 따라 유저를 자동으로 분류합니다:

| 타겟 (Segment) | 분류 기준 (Logic) | 운영 전략 |
| :--- | :--- | :--- |
| **VIP** | 총 운영 마진 > 1,000,000 | 전담 케어 및 전용 혜택 |
| **AT_RISK** | 미접속 > 7일 AND 마진 > 0 | 이탈 방지 넛지 및 복귀 쿠폰 |
| **WHALE** | 누적 충전 금액 > 5,000,000 | 고액 베터 전용 이벤트 |
| **COMMON** | 그 외 일반 사용자 | 기본 리텐션 관리 |

*   **샘플 다운로드**: [sample_hq_margin.csv](file:///C:/Users/JAVIS/ch/ch25/docs/v2_specs/CSV_Samples/sample_hq_margin.csv)

##### **D. 인코딩 및 주의사항 (Encoding)**
*   **인코딩 자동 감지 (Smart Detection)**:
    *   V2 시스템은 업로드된 파일의 인코딩을 자동으로 감지합니다. 
    *   **CP949(한국형 엑셀)**와 **UTF-8**을 모두 지원하므로, 인코딩 변환 없이 바로 업로드 가능합니다.
