...
## 7. 운용 주기 및 시간대 정책 (System Time Cycle)

### 7.1 표준 시간대 (Standard Timezone)
- **Asia/Seoul (KST)**: 모든 비즈니스 로직, 미션 리셋, 로그 기록의 기준 시간대입니다.
- 서버의 로컬 시간이나 시스템 시간(UTC)을 직접 사용하지 않고, 항상 `Asia/Seoul`로 변환하여 처리합니다.

### 7.2 운용일 (Operational Day)
- **기준점**: 매일 **오전 09:00:00 KST**.
- **정의**: 오늘 09:00부터 익일 08:59:59까지를 하나의 '운용일'로 간주합니다.
- **적용 범위**: 미션 리셋, 스트릭 카운팅, 출금 한도 리셋, 데일리 기프트 지급 등.
- **목적**: 자정 리셋 시 발생하는 세션 분리 및 데이터 불일치 문제를 방지하고 운영 편의성을 확보합니다.

---

## 8. 변경 이력
- v2.2 (2026-01-22, GitHub Copilot): 운용일(9AM 리셋) 및 KST 표준 시간대 정책 추가.
