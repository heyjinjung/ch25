...
## 3. 참여 자격 및 기간 (Finalized)
1. **대상**: 신규 가입한 모든 유저 (`days_since_signup` 계계산 시점 기준)
2. **기간**: 가입 시점(`created_at`)으로부터 **정확히 7일 (168시간)**
   - W05 정책 결정에 따라 기존 72시간에서 7일로 확정 연장됨.
3. **만료 처리**:
   - 168시간 경과 시 "신규 유저" 탭은 즉시 비활성화되며, 진행 중인 미션의 보상 수령도 불가함.

### 3.1. FAB(Floating Action Button) 타이머 UX 가이드라인
신규 유저의 긴박감(FOMO) 유발과 혜택 인지력 향상을 위해 아래 UX 요건을 강제한다.
- **노출 트리거**: 유저가 신규 유저 미션 자격이 있을 때만 메인 하단에 상시 노출.
- **디자인**:
  - **Shape**: 원형 또는 알약 모양의 플로팅 버튼.
  - **Color**: Golden V2 테마에 맞춘 Gold/Yellow Gradient 배경에 검은색 텍스트.
  - **Content**: `남은 시간: HH:MM:SS` (실시간 카운트다운).
- **인터랙션**:
  - 클릭 시 즉시 `MissionsPage`의 `New User` 탭으로 이동.
  - 만료 1시간 전부터는 버튼이 빨간색으로 점멸(Flash) 애니메이션 효과 적용.
