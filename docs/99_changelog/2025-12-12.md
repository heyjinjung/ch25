...
- 시즌패스 ↔ 코어 레벨 동기화: `add_stamp`/`add_bonus_xp`가 `LevelXPService.add_xp` 호출.
...
- RewardService: 순환 의존성 방지를 위해 시즌패스 import 지연.
- 게임 확률 조정: 룰렛/복권 등 꽝 비중 상향으로 과도 보상 완화.
- 외부랭킹 UI: 저장/삭제 후 행 유지/반영하도록 캐시 업데이트 수정.

## 문서
- [docs/level_xp_core_rewards.md](../level_xp_core_rewards.md): 코어 레벨/XP 보상안, DB/마이그레이션, 시즌패스 연동, 외부랭킹 안티어뷰즈 환경 변수 추가.
