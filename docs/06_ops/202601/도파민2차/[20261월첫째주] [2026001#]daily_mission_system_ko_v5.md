...
✅## 2.0.1 신규회원(웰컴) 이벤트 미션: “미션 4개 = 총 1만원” (구현 기준)

> 목표: 신규 유저에게 “지금 당장 할 것”을 주되, 보상은 **즉시 현금화되지 않도록** Vault 정책(출금 조건) 아래에 둔다.

✅#### 지급 구조(현재 구현)
- **보상 형태**: 각 미션 클레임 시 `vault_locked_balance`로 **적립(accrue)**
  - `Mission.reward_type=CASH_UNLOCK`이지만, 실제 동작은 “unlock”이 아니라 **Vault locked 적립**으로 리팩터링됨
- **총 보너스 상한**: 2,500원 × 4개 = **유저당 최대 10,000원**
- **출금/현금화**: 웰컴 미션 자체에 “입금 필수” 조건은 없고,
  출금 가능 여부는 금고 정책(예: **당일 입금 필요**, 최소 출금액 등)에 의해 결정됨

✅#### 웰컴 미션 4종(현재 시드)
- `NEW_USER_PLAY_1` (action: `PLAY_GAME`, target=1) → 2,500
- `NEW_USER_PLAY_3` (action: `PLAY_GAME`, target=3) → 2,500
- `NEW_USER_VIRAL` (action: `JOIN_CHANNEL`, target=1) → 2,500
- `NEW_USER_LOGIN_DAY2` (action: `LOGIN`, target=2) → 2,500

✅#### 트리거(진행도 업데이트 경로)
- 게임 플레이: `RouletteService/DiceService/LotteryService` → `MissionService.update_progress(user_id, "PLAY_GAME")`
- 채널/바이럴: `POST /api/viral/join-channel` → `MissionService.update_progress(user_id, "JOIN_CHANNEL")`
- 로그인: `/api/auth/token` → `MissionService.update_progress(user_id, "LOGIN")`

✅#### 24시간 윈도우(UX)
- `/api/new-user/status`가 `first_login_at` 기준 **24시간 window**를 계산해 내려줌
- 현재 백엔드는 NEW_USER 미션 자체를 강제로 만료 처리하진 않으며, 프론트에서 window 종료 시 UX로 제어하는 방식
...
- **성공 경험 선지급**: 복잡한 게임 미션 이전에 “입장 완료” 보상을 먼저 주어 유저에게 ‘진척감’ 부여.
  ...
  **TRIAL_GRANT 정책(서버/프론트 공통 방어, 2026-01 반영)**
  - 엔드포인트/기능명은 `trial-grant`를 유지하지만, **실전 티켓 직접 지급은 하지 않음**
  - 요청 토큰이 `ROULETTE_COIN` / `DICE_TOKEN` / `LOTTERY_TICKET` 중 하나이고 잔액이 0이면:
    - **`TRIAL_TOKEN` 3장**을 지급(기존 실전 티켓 지급을 `TRIAL_TOKEN`으로 리다이렉트)
    - 이미 `TRIAL_TOKEN`이 남아있으면 추가 지급하지 않음(먼저 소비 유도)
  - `GOLD_KEY` / `DIAMOND_KEY`는 trial-grant로 **지급되지 않음**(요청되어도 서버에서 차단)
  - 유저 1인당 **일일 총 지급 상한** 존재(ledger reason=`TRIAL_GRANT` 기준)
  - 추가 레버(서버 env): `ENABLE_TRIAL_GRANT_AUTO`, `TRIAL_DAILY_CAP`, `TRIAL_WEEKLY_CAP`, `TRIAL_GRANT_*`(확률/첫회 보장)
