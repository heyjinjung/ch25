...
- **기본 CRUD API** (작성/조회/검색/Export 트리거 등) + 권한(최소 Role 기반)
- 안전 규칙 구현: `ref_id` 멱등 처리, PII 마스킹, 수정/삭제 대신 `CORRECTION` 방식
- 어드민의 “수동 기록”용 **Quick Ops Logger 1차 버전(MVP)** (기능 토글/Confirm 포함)
...
### 3-4. Export & Reporting (마감 자동화)

#### A. 일일 리포트 자동 생성

`OpsDailyLog.status`가 `PUBLISHED`로 전환될 때 자동 실행:

1. **KPI 스냅샷 고정**: `kpi_snapshot` JSON 필드에 마감 시점 데이터 저장.
2. **마크다운 Export**: 기존 `[YYYYMMDD]_운영일지.md` 포맷으로 변환 및 파일 저장.
3. **Slack/Telegram 공유**: "오늘의 운영 요약" 메시지 자동 발송.

#### B. 주간/월간 통합 리포트

- 매주 월요일 00:00 자동 집계 → **Weekly Summary** 생성.
- 로그 데이터 기반 **Trend Chart** (DAU, Revenue, Churn Rate 추이).
- 세그먼트별 **Conversion Funnel** 시각화.
...
3. **Log Listener (`app/listeners/ops_log_listener.py`)**:
  ...
  **(신규) 운영로그 모듈 라우트 제안(컨벤션 정렬)**
  ...
  - `POST /admin/api/ops/exports/{date}` (운영일지 마크다운 export 트리거)
...
**Phase 1 (연동: Export/템플릿/권한 강화)**

- Export 트리거/다운로드: `POST /admin/api/ops/exports/{date}`
