...
| **Username** | `root` |
| **Password** | `REDACTED` (문서/레포에 비밀번호 금지 — 노출 시 즉시 회전) |
| **OS** | Ubuntu 22.04 x64 |
| **Label** | xmas |
| **Location** | Singapore |
...
## 12) Git 운영 기준 (로컬 2계정/서버 빌드 기준)

로컬에 Git 계정이 2개(예: `혜진정` / `마마1281`) 존재하는 경우, IDE 자동 커밋/잘못된 계정 서명으로 브랜치가 꼬이기 쉬우므로 아래 기준으로 운영합니다.

- **기준 리모트/브랜치**
  - 로컬 개발 기준: `마마1281` 쪽 최신 커밋을 기준으로 작업
  - PR 기준: 최종적으로 `혜진정` 계정(원본)으로 Pull Request 발행
  - 서버(SSH) 빌드 기준: `마마1281` 최신 커밋을 기준으로 배포/빌드
...
- **사고 방지 권장 설정(로컬 레포 단위)**
  - `git config --local user.useConfigOnly true`
  - (필요 시) 레포별로 `git config --local user.name "..."`, `git config --local user.email "..."` 고정
  - VS Code 설정에서 **Smart Commit(메시지 없이 커밋)** 류 기능은 비활성화 권장
